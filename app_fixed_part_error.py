...
AI3D_HOST = "ai3d.tencentcloudapi.com"  # для новых AI3D-операций (FBX-конвертация, Part) — НЕ intl!
AI3D_SERVICE = "ai3d"
...
TEXTS = {
    "ru": {
        ...
        "part_splitting": "🧩 Разбиваю модель на части с шипами и пазами...\n⏳ Это займёт 2-4 минуты.",
    ...
    "en": {
        ...
        "part_splitting": "🧩 Splitting model into parts with pins and sockets...\n⏳ This takes 2-4 minutes.",
...
user_paid_one = {}
user_last_prompt = {}
user_last_image = {}  # user_id -> image_base64 последней фото-генерации (для Part fallback)
user_last_glb_url = {}  # user_id -> GLB URL последней модели (для PartSplit)
user_last_hunyuan_job = {}  # user_id -> JobId (для перезапроса свежего URL)
...
def get_ai3d_headers(action, payload):
    """TC3-HMAC-SHA256 подпись для нового AI3D API (Convert3DFormat FBX, Part, и др.)
    host=ai3d.intl.tencentcloudapi.com, service=ai3d, version=2025-05-13
    """
...
def hunyuan_generate_from_photo(image_base64, output_format="stl", max_retries=3, user_id=None):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if user_id:
                user_last_hunyuan_job[user_id] = job_id
                user_last_image[user_id] = image_base64  # для fallback-регенерации через FBX
...
def hunyuan_regenerate_for_part(prompt=None, image_base64=None):
    """Fallback: регенерирует модель с ResultFormat='FBX' — сразу получаем FBX без Convert3DFormat.
    Использует AI3D endpoint для поддержки FBX на выходе.

    Возвращает: fbx_url (str) или None если не удалось.
    """
    print("[PartFallback] Регенерирую модель с ResultFormat=FBX...")
    try:
        action = "SubmitHunyuanTo3DProJob"
        payload_dict = {
            "Model": "3.1",
            ...
            "GenerateType": "Normal",
            "FaceCount": 1000000,
            "ResultFormat": "FBX"  # ★ сразу FBX на выходе ★
        }
        if prompt:
            ...
            print("[PartFallback] Нет ни prompt, ни image — fallback невозможен")
            return None

        payload = json.dumps(payload_dict)
        headers = get_ai3d_headers(action, payload)  # ★ AI3D endpoint ★
        resp = safe_request("post", f"https://{AI3D_HOST}", headers=headers, data=payload, timeout=90)
        data = resp.json()

        print(f"[PartFallback] Submit ответ: {json.dumps(data, ensure_ascii=False)[:300]}")

        if "Response" not in data or "JobId" not in data["Response"]:
            print(f"[PartFallback] Нет JobId: {data}")
            return None

        job_id = data["Response"]["JobId"]
        print(f"[PartFallback] JobId={job_id}, опрашиваю...")

        query_action = "QueryHunyuanTo3DProJob"
        for _ in range(40):
            time.sleep(5)
            q_payload = json.dumps({"JobId": job_id})
            q_headers = get_ai3d_headers(query_action, q_payload)
            q_resp = safe_request("post", f"https://{AI3D_HOST}", headers=q_headers, data=q_payload, timeout=90)
            q_data = q_resp.json()
            status = q_data.get("Response", {}).get("Status", "")
            print(f"[PartFallback] Query status: {status}")
            if status == "DONE":
                result_files = q_data["Response"].get("ResultFile3Ds", [])
                for f in result_files:
                    if f.get("Format", "").lower() == "fbx":
                        fbx_url = f.get("Url")
                        if fbx_url:
                            print(f"[PartFallback] FBX URL получен: {fbx_url[:80]}...")
                            return fbx_url
                # Если FBX не найден — берём первый URL
                if result_files:
                    fbx_url = result_files[0].get("Url")
                    print(f"[PartFallback] FBX URL (первый файл): {fbx_url[:80] if fbx_url else 'None'}...")
                    return fbx_url
                print("[PartFallback] ResultFile3Ds пуст")
                return None
            elif status == "FAIL":
                err = q_data["Response"].get("ErrorMessage", "?")
                print(f"[PartFallback] Job FAILED: {err}")
                return None

        print("[PartFallback] Таймаут поллинга (200 сек)")
        return None
    except Exception as e:
        print(f"[PartFallback] Exception: {e}")
        return None
...
def process_part_generation(chat_id, user_id, glb_url, gen_type, job_id=None, prompt=None, image_base64=None):
    """Фоновая разбивка: конвертирует GLB в FBX, затем разбивает на части.

    ★ ИСПРАВЛЕНО ★:
    1. Convert3DFormat FBX → AI3D endpoint (ai3d.tencentcloudapi.com)
       Старый Hunyuan endpoint возвращает ResourceUnavailable.InterfaceNotExist.
    2. Перезапрос Hunyuan job для свежего GLB URL.
    3. Fallback: если конвертация не удалась → регенерируем модель с ResultFormat='FBX'.
    """
    try:
        # 0: Перезапрашиваем Hunyuan job, чтобы получить свежий GLB URL
        if job_id:
            print(f"[Part] Перезапрашиваю job_id={job_id} для свежего URL...")
            try:
                recheck_action = "QueryHunyuanTo3DProJob"
                recheck_payload = json.dumps({"JobId": job_id})
                recheck_headers = get_tencent_headers(recheck_action, recheck_payload)
                recheck_resp = safe_request("post", f"https://{HUNYUAN_HOST}",
                                           headers=recheck_headers, data=recheck_payload, timeout=90)
                recheck_data = recheck_resp.json()
                if "Response" in recheck_data and recheck_data["Response"].get("Status") == "DONE":
                    fresh_url = recheck_data["Response"].get("ResultFile3Ds", [{}])[0].get("Url")
                    if fresh_url:
                        print(f"[Part] Получен свежий URL: {fresh_url[:80]}...")
                        glb_url = fresh_url
                    else:
                        print("[Part] Свежий URL не получен, использую сохранённый")
                else:
                    print(f"[Part] Job status: {recheck_data.get('Response', {}).get('Status', '?')}")
            except Exception as req_e:
                print(f"[Part] Не удалось перезапросить job: {req_e}, использую сохранённый URL")

        # 1: Конвертируем GLB в FBX через Convert3DFormat ★ на AI3D endpoint ★
        send_message(chat_id, t(user_id, "part_converting"))

        fbx_url = None
        last_error = None

        # --- Попытка через Convert3DFormat на AI3D endpoint ---
        for attempt in range(3):
            try:
                print(f"[Part] Convert3DFormat GLB→FBX на AI3D, попытка {attempt+1}/3")
                conv_action = "Convert3DFormat"
                conv_payload = json.dumps({"File3D": glb_url, "Format": "FBX"})
                # ★ КЛЮЧЕВОЕ ИСПРАВЛЕНИЕ: используем AI3D endpoint ★
                ...
                print(f"[Part] Convert3DFormat ответ: {json.dumps(conv_data, ensure_ascii=False)[:500]}")

                if "Response" in conv_data:
                    resp = conv_data["Response"]
                    if "Error" in resp:
                        api_err = resp["Error"]
                        last_error = f"API Error: {api_err.get('Code', '?')} — {api_err.get('Message', '?')}"
                        print(f"[Part] Convert3DFormat API ошибка: {last_error}")
                        if "expired" in str(api_err).lower() or "invalid" in str(api_err).lower():
                            time.sleep(3)
                            continue
                        # Не-retryable ошибка — выходим из цикла конвертации, пробуем fallback
                        break

                    if "ResultFile3D" in resp:
                        fbx_url = resp["ResultFile3D"]
                        print(f"[Part] FBX URL получен: {fbx_url[:80]}...")
                        break
                    else:
                        last_error = f"ResultFile3D отсутствует. Ключи: {list(resp.keys())}"
                        print(f"[Part] {last_error}")
                else:
                    last_error = f"Response отсутствует. Ключи: {list(conv_data.keys())}"
                    print(f"[Part] {last_error}")

                time.sleep(3)
            except Exception as e:
                last_error = str(e)
                print(f"[Part] Convert3DFormat exception: {e}")
                time.sleep(5)

        # --- Fallback: регенерируем модель с ResultFormat='FBX' ---
        if not fbx_url and (prompt or image_base64):
            print(f"[Part] Convert3DFormat не сработал ({last_error}), запускаю fallback-регенерацию...")
            send_message(chat_id, t(user_id, "part_fallback"))

            fbx_url = hunyuan_regenerate_for_part(prompt=prompt, image_base64=image_base64)
            if fbx_url:
                print(f"[Part] Fallback-регенерация успешна: {fbx_url[:80]}...")
            else:
                print("[Part] Fallback-регенерация тоже не удалась")

        if not fbx_url:
            raise Exception(
                f"Не удалось получить FBX для разбивки.\n"
                f"Причина конвертации: {last_error}\n"
                f"GLB URL: {glb_url[:100]}...\n"
                f"Рекомендация: попробуйте сгенерировать модель заново и сразу нажать «Разбить»."
            )
                ...
                for update in updates:
                    ...
                    if "callback_query" in update:
                        ...
                        # ★ НОВЫЕ ФИЧИ ★
                            ...
                            executor.submit(process_part_generation, chat_id, user_id, glb_url, gen_type, job_id,
                                           user_last_prompt.get(user_id),
                                           user_last_image.get(user_id))
