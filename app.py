...
# Meshy опционален — бот работает и без него
if MESHY_API_KEY:
...
user_service_choice = {}  # user_id -> "hunyuan" или "meshy"
...
def meshy_generate_with_retry(prompt, max_retries=2):
    """Генерация через Meshy API. Возвращает bytes модели."""
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Шаг 2: Ждём результат (максимум 20 попыток)
            for _ in range(20):
                time.sleep(5)
                ...
                if status == "SUCCEEDED":
                    # Шаг 3: Запускаем улучшение (refine)
                    refine_payload = {
                        "mode": "refine",
...
def process_photo_generation(chat_id, user_id, image_base64, gen_type):
    # Фото-генерация пока только через Hunyuan
    try:
...
# ========== HUNYUAN PHOTO (оставляем без изменений) ==========
...
def handle_help_info(chat_id):
    text = (
        ...
        "⚡ *Выбор сервиса:* Нажмите «Выбрать сервис» в меню, чтобы переключиться между Hunyuan и Meshy"
    )
