...
# ========== HUNYUAN API (С ЗАЩИТОЙ ОТ ТАЙМ-АУТА) ==========
...
def safe_request(method, url, max_retries=3, **kwargs):
    """Безопасный запрос с автоповтором при Timeout"""
            ...
            if resp.status_code == 200:
                for update in resp.json().get("result", []):
                    ...
                    if "message" in update:
                        ...
                        # Команда /stats (только админ) — ТЕПЕРЬ С ID
                        ...
                        # Пользователь в режиме ожидания генерации
                        if chat_id in user_states:
