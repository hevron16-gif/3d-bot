...
print("🟢 [DEBUG] Импорты выполнены", flush=True)
...
print(f"🟢 [DEBUG] TELEGRAM_BOT_TOKEN: {'✅' if TELEGRAM_BOT_TOKEN else '❌'}", flush=True)
...
print("🟢 [DEBUG] Переменные загружены", flush=True)


# ========== ДЛЯ ОТВЕТОВ АДМИНА ==========
pending_reply = {}  # {chat_id: user_id} — чтобы знать, кому отвечать
...
def forward_to_admin(user_id, username, first_name, text):
    """Пересылает сообщение пользователя админу и запоминает, кому отвечать"""
    if ADMIN_CHAT_ID:
        try:
            name = first_name or ""
            user_tag = f"@{username}" if username else f"ID: {user_id}"
            
            # Запоминаем, что следующий ответ админа пойдёт этому пользователю
...
def reply_to_user(admin_message_text):
    """Отправляет ответ пользователю, если админ пишет в чат с ботом"""
    if ADMIN_CHAT_ID in pending_reply:
        ...
        # Не удаляем, чтобы можно было продолжать диалог
    else:
...
def hunyuan_generate_with_retry(prompt, max_retries=3):
    """Генерация с автоматическими повторами при ошибке"""
    for attempt in range(max_retries):
        try:
            print(f"Генерация для пользователя, попытка {attempt+1}")
            ...
            if resp.status_code == 200:
                for update in resp.json().get("result", []):
                    ...
                    if "message" in update:
                        ...
                        if user_id == ADMIN_CHAT_ID and chat_id == ADMIN_CHAT_ID:
                            if "text" in msg and msg["text"]:
                                reply_to_user(msg["text"])
                            continue
                        
                        # Пересылка сообщений от обычных пользователей админу
                        ...
                        if chat_id in user_states and user_states[chat_id] == "awaiting_text" and "text" in msg:
                            del user_states[chat_id]
                            prompt = msg["text"]
                        ...
                        elif chat_id in user_states and user_states[chat_id] == "awaiting_photo" and "photo" in msg:
                            del user_states[chat_id]
                            ...
                            if chat_id not in user_states and "text" in msg and not msg["text"].startswith('/'):
                                send_message(chat_id, "Сначала нажмите кнопку в меню.", keyboard=main_keyboard)
            time.sleep(1)
        except Exception as e:
