...
# Новый API для Multi-View (поддерживает MultiViewImages и ResultFormat)
...
user_blueprint_images = {}  # user_id -> list из 3 base64: [front, left, top]
...
# ========== HUNYUAN MULTI-VIEW API (новый endpoint ai3d) ==========
def get_ai3d_headers(action, payload):
    """
    Формирует заголовки для НОВОГО Tencent AI3D API (v2025-05-13).
    Только этот endpoint поддерживает MultiViewImages, ResultFormat.
    Документация: https://cloud.tencent.com/document/api/1804/123447
    """
...
def hunyuan_generate_from_drawings(prompt, front_b64, left_b64, top_b64, output_format='stl', max_retries=3):
    """
    Генерация 3D-модели из трёх чертежей (Multi-View).

    Параметры:
        prompt: str — текстовое описание модели (до 900 символов)
        front_b64: str — base64 фронтального вида (ОБЯЗАТЕЛЕН,
                   передаётся как ImageBase64)
        left_b64: str — base64 левого вида (→ ViewType: 'left')
        top_b64: str — base64 верхнего вида (→ ViewType: 'top')
        output_format: str — 'stl', 'glb', 'obj', 'usdz', 'fbx'
        max_retries: int — количество попыток при ошибке

    Возвращает: (bytes_модели, str_формат)

    Использует НОВЫЙ AI3D API (ai3d.intl.tencentcloudapi.com, v2025-05-13),
    так как старый hunyuan endpoint не поддерживает MultiViewImages.
    
    Допустимые ViewType (Model 3.1):
      left, right, back — базовые
      top, bottom — 3.1+
      left_front, right_front — 3.1+ (45°)
    """
    # Приводим формат к верхнему регистру для ResultFormat
    result_fmt = output_format.upper()

    for attempt in range(max_retries):
        try:
            print(f"[MultiView] Попытка {attempt+1}/{max_retries}, формат: {result_fmt}")

            # === payload_dict — В ТОЧНОСТИ как в спецификации Tencent ===
            payload_dict = {
                'Model': '3.1',
                'Prompt': prompt[:900],
                'EnablePBR': True,
                ...
                'MultiViewImages': [
                    {'ViewType': 'left', 'ViewImageBase64': left_b64},
                    {'ViewType': 'top', 'ViewImageBase64': top_b64}
                ],
                'ResultFormat': result_fmt
            }

            # Отправляем задание через НОВЫЙ API
            action = "SubmitHunyuanTo3DProJob"
            payload = json.dumps(payload_dict)
            headers = get_ai3d_headers(action, payload)
            resp = safe_request("post", f"https://{AI3D_HOST}",
                              headers=headers, data=payload, timeout=90)
            data = resp.json()

            # Проверяем ответ на ошибки
            if "Response" not in data:
                raise Exception(f"[MultiView] Нет Response в ответе: {data}")
            if "Error" in data["Response"]:
                err = data["Response"]["Error"]
                code = err.get("Code", "")
                msg = err.get("Message", "")
                raise Exception(
                    f"[MultiView] Ошибка API [{code}]: {msg}\n"
                    f"Возможные причины:\n"
                    f"  1. Неверный формат ViewImage (ViewType/ViewImageBase64)\n"
                    f"  2. Слишком большие изображения (>6 MB base64)\n"
                    f"  3. Неподдерживаемый ViewType (допустимы: left,right,back,top,bottom,left_front,right_front)\n"
                    f"  4. Prompt и ImageBase64 одновременно (не поддерживается вне Sketch-режима)"
                )
            if "JobId" not in data["Response"]:
                raise Exception(f"[MultiView] Нет JobId в ответе: {data}")

            job_id = data["Response"]["JobId"]
            print(f"[MultiView] Задание создано: {job_id}")

            # Ожидаем завершения (polling)
            query_action = "QueryHunyuanTo3DProJob"
            for _ in range(40):  # 40 × 5 сек = 200 сек макс
                time.sleep(5)
                payload = json.dumps({"JobId": job_id})
                headers = get_ai3d_headers(query_action, payload)
                resp = safe_request("post", f"https://{AI3D_HOST}",
                                  headers=headers, data=payload, timeout=90)
                data = resp.json()
                ...
                if "Response" in data:
                    ...
                    if status == "DONE":
                        result_files = data["Response"].get("ResultFile3Ds", [])
                        if not result_files:
                            raise Exception("[MultiView] ResultFile3Ds пуст после DONE")

                        # Ищем файл с нужным Type (напр. "STL", "GLB")
                        file_url = None
                        for f in result_files:
                            if (f.get("Type") or "").upper() == result_fmt:
                                file_url = f.get("Url")
                                break
                        # Fallback: берём первый URL
                        if not file_url:
                            file_url = result_files[0].get("Url")
                            print(f"[MultiView] {result_fmt} не найден по Type, беру первый файл")

                        if not file_url:
                            raise Exception("[MultiView] Нет URL в ResultFile3Ds")

                        # Скачиваем модель
                        model_resp = safe_request("get", file_url, timeout=120)
                        if model_resp.status_code != 200:
                            raise Exception(
                                f"[MultiView] Не удалось скачать модель: HTTP {model_resp.status_code}"
                            )

                        model_data = model_resp.content
                        print(f"[MultiView] ✅ Модель готова: {len(model_data)} байт, формат: {output_format}")
                        return model_data, output_format

                    elif status == "FAIL":
                        error_msg = data["Response"].get("ErrorMessage", "неизвестная ошибка")
                        raise Exception(f"[MultiView] Генерация FAILED: {error_msg}")
...
# ========== ГЕНЕРАЦИЯ ПО ЧЕРТЕЖАМ (MULTI-VIEW) ==========
def process_blueprint_generation(chat_id, user_id, front_b64, left_b64, top_b64, gen_type):
    """Фоновая генерация 3D-модели по трём чертежам (Multi-View)."""
    output_format = user_format_choice.get(user_id, "stl")
    # prompt по умолчанию — базовое описание
    prompt = "A 3d model based on three views: front, left side, top"
    try:
        model, fmt = hunyuan_generate_from_drawings(
            prompt, front_b64, left_b64, top_b64, output_format
        )
        if model:
            use_generation(user_id, gen_type)
            update_stats(user_id)
            caption = t(user_id, "model_ready", service="HUNYUAN (Multi-View)",
                        fmt=fmt.upper(), prompt="Чертёж")
            if gen_type == "free" and user_id != ADMIN_CHAT_ID:
                remaining = max(0, FREE_LIMIT - user_free_used.get(user_id, 0))
                caption += t(user_id, "free_remaining", remaining=remaining, limit=FREE_LIMIT)
            filename = f"model.{fmt}"
        ...
        send_message(chat_id, t(user_id, "error", error=str(e)))
        send_alert(f"Ошибка Multi-View генерации у {user_id}: {str(e)}")
    finally:
        user_busy[user_id] = False
        # Очищаем сохранённые изображения чертежа
...
def get_main_keyboard(user_id):
    ...
    return {
        "inline_keyboard": [
            ...
            [{"text": "📐 По чертежу" if lang == "ru" else "📐 By Blueprint", "callback_data": "gen_blueprint"}],
                ...
                for update in updates:
                    ...
                    if "callback_query" in update:
                        ...
                        if data == "menu":
                            ...
                            # ★ Запуск режима загрузки чертежей ★
                            user_states[chat_id] = "awaiting_blueprint"
                            user_blueprint_images[user_id] = []  # [front, left, top]
                            send_message(chat_id,
                                "📐 <b>Режим генерации по чертежу</b>\n\n"
                                "Отправьте <b>ТРИ</b> фото последовательно:\n"
                                "1️⃣ Фронтальный вид (спереди)\n"
                                "2️⃣ Левый вид (сбоку)\n"
                                "3️⃣ Верхний вид (сверху)\n\n"
                                "⚠️ Отправляйте именно фото, по одному за раз.",
                    ...
                    if "message" in update:
                        ...
                        if chat_id in user_states:
                            ...
                            if state == "awaiting_text" and "text" in msg:
                            ...
                            elif state == "awaiting_blueprint" and "photo" in msg:
                                # ★ Приём чертежей: 3 фото по одному ★
                                bp_images = user_blueprint_images.get(user_id, [])
                                try:
                                    file_id = msg["photo"][-1]["file_id"]
                                    file_info = requests.get(API_URL + f"/getFile?file_id={file_id}").json()
                                    file_url = f"https://api.telegram.org/file/bot{TELEGRAM_BOT_TOKEN}/{file_info['result']['file_path']}"
                                    ...
                                    bp_images.append(image_b64)
                                    user_blueprint_images[user_id] = bp_images
                                except Exception as e:
                                    send_message(chat_id, f"❌ Ошибка загрузки фото: {str(e)}")
                                    continue
                                ...
                                count = len(bp_images)
                                if count == 1:
                                    send_message(chat_id, "✅ Фронтальный вид получен.\n📸 Отправьте <b>левый вид (сбоку)</b>.")
                                elif count == 2:
                                    send_message(chat_id, "✅ Левый вид получен.\n📸 Отправьте <b>верхний вид (сверху)</b>.")
                                elif count >= 3:
                                    # Все 3 фото получены — запускаем генерацию
                                    del user_states[chat_id]
                                    rate_ok, rate_msg = check_rate_limit(user_id)
                                    if not rate_ok:
                                        send_message(chat_id, t(user_id, "rate_limit", msg=rate_msg), keyboard=get_main_keyboard(user_id))
                                    ...
                                    user_busy[user_id] = True
                                    fmt = user_format_choice.get(user_id, "stl")
                                    send_message(chat_id, t(user_id, "generating", service="Hunyuan (Multi-View)", fmt=fmt.upper()))
                                    front_b64 = bp_images[0]
                                    left_b64 = bp_images[1]
                                    top_b64 = bp_images[2]
                                    executor.submit(process_blueprint_generation, chat_id, user_id, front_b64, left_b64, top_b64, gen_type)
                                continue
