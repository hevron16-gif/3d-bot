...
user_states = {}
user_limits = {}          # {user_id: (date, meshy_count, hunyuan_count)}
user_paid_one = {}        # {user_id: date} разовая покупка
user_subscription = {}    # {user_id: expiry_date} подписка
...
def can_generate(user_id, engine):
    ...
    # Разовый платёж сегодня
    if user_paid_one.get(user_id) == today:
        return True
    
    # Проверка бесплатных лимитов
    ...
    if engine == "meshy":
        return meshy_count < 20  # 20 моделей в день через Meshy
    else:
        return hunyuan_count < 10  # 10 моделей в день через Hunyuan
...
def use_generation(user_id, engine):
    ...
    # Подписка или разовый платёж — не уменьшаем лимит
                ...
                for update in resp.json().get("result", []):
                    ...
                    if "message" in update and "successful_payment" in update["message"]:
                        ...
                        if payload == "single_generation":
                            ...
                            send_message(user_id, "✅ Разовая генерация активирована! Теперь выбери движок и отправь текст или фото.")
                    ...
                    if "callback_query" in update:
                        ...
                        if chat_id in user_states:
                            ...
                            if not can_generate(user_id, engine):
                                send_message(chat_id, f"❌ Бесплатный лимит для {engine.upper()} на сегодня закончился. Купи разовую генерацию или подписку.", keyboard=main_keyboard)
                                continue
