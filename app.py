...
main_keyboard = {
    "inline_keyboard": [
        ...
        [{"text": "📦 История", "callback_data": "my_models"}],
...
def handle_buy_one(chat_id):
    send_message(chat_id, "🎟 Разовая генерация стоит 100₽.\n\nОплата (Telegram Stars или карта) будет добавлена позже. Пока используй бесплатные генерации.", keyboard=back_keyboard)


def handle_subscription(chat_id):
    send_message(chat_id, "💎 Подписка на месяц — 299₽ (безлимит).\n\nОплата появится в ближайшее время. Следи за обновлениями!", keyboard=back_keyboard)
