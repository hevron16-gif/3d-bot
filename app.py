...
def hunyuan_generate_from_text(prompt, retry=2):
    if len(prompt) > 900:
        prompt = prompt[:900] + "..."
    ...
    for attempt in range(retry):
        try:
            ...
            if attempt < retry - 1:
                ...
                time.sleep(10)
                continue
            raise Exception("Hunyuan timeout after retries")
        except Exception as e:
            if attempt == retry - 1:
                raise e
            print(f"Ошибка в попытке {attempt+1}: {e}, повтор через 10 секунд...")
            time.sleep(10)
    raise Exception("Hunyuan generation failed after retries")


def hunyuan_generate_from_photo(image_base64, retry=3):
    for attempt in range(retry):
        try:
            print(f"Фото: попытка {attempt+1} из {retry}")
            action = "SubmitHunyuanTo3DProJob"
            payload = json.dumps({"ImageBase64": image_base64})
            headers = get_tencent_headers(action, payload)
            # Увеличенный таймаут для фото
            resp = requests.post(f"https://{HUNYUAN_HOST}", headers=headers, data=payload, timeout=120)
            data = resp.json()
            if "Response" not in data or "JobId" not in data["Response"]:
                raise Exception(f"Hunyuan submit error: {data}")
            ...
            for _ in range(90):
                ...
                if "Response" in data:
                    ...
                    if status == "DONE":
                        ...
                        raise Exception("Generation failed")
            if attempt < retry - 1:
                print(f"Фото: попытка {attempt+1} не удалась, повтор через 10 секунд...")
                time.sleep(10)
                continue
            raise Exception("Hunyuan timeout after retries")
        except Exception as e:
            if attempt == retry - 1:
                raise e
            print(f"Фото: ошибка в попытке {attempt+1}: {e}, повтор через 10 секунд...")
            time.sleep(10)
    raise Exception("Hunyuan generation failed after retries")
...
def handle_help_info(chat_id):
    text = (
        ...
        "3️⃣ Подождите 1-5 минут — модель готова!\n\n"
        ...
        "⚠️ Генерация по фото может занимать до 4-5 минут."
    )
                ...
                for update in resp.json().get("result", []):
                    ...
                    if "callback_query" in update:
                        ...
                        if data == "menu":
                            ...
                            send_message(chat_id, "📦 История моделей появится позже.", keyboard=back_keyboard)
                        elif data == "gen_hunyuan":
                            user_states[chat_id] = "hunyuan"
                            send_message(chat_id, "🔧 Выберите способ:\n\n📝 Отправьте ТЕКСТ с описанием модели\n📸 Или отправьте ФОТО объекта\n\n⚠️ Генерация по фото может занять до 4-5 минут.", keyboard=back_keyboard)
