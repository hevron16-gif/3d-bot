    ...
    if "message" in update:
        ...
        if text == "/start":
            ...
            try:
                ...
                payload = {"prompt": text, "mode": "preview"}
                response = requests.post("https://api.meshy.ai/openapi/v1/text-to-3d", headers=headers, json=payload, timeout=30)
                
                requests.post(API_URL + "/sendMessage", json={
                    "chat_id": chat_id,
                    "text": f"Статус Meshy: {response.status_code}\nОтвет: {response.text[:400]}"
                })
