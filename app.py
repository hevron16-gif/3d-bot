...
# ========== ПЕРЕСЫЛКА И ОТВЕТЫ (ИСПРАВЛЕНО) ==========
def forward_to_admin(user_id, username, first_name, text):
    print(f"!!! [DEBUG] forward_to_admin вызвана: user_id={user_id}, text={text[:50]}", flush=True)
    if not ADMIN_CHAT_ID:
        print("⚠️ ADMIN_CHAT_ID не задан", flush=True)
        return
    try:
        name = first_name or ""
        ...
        print(f"!!! [DEBUG] Результат отправки админу: {resp.status_code}", flush=True)
        
        if resp.status_code != 200:
            print(f"!!! [DEBUG] Ошибка отправки: {resp.text[:200]}", flush=True)
            requests.post(API_URL + "/sendMessage", json={
                "chat_id": ADMIN_CHAT_ID,
...
# ========== ЛИМИТЫ (ИСПРАВЛЕНО: 3 бесплатные) ==========
FREE_LIMIT = 3  # Было 5, стало 3
...
# ========== MESHY API ==========
def meshy_generate_with_retry(prompt, max_retries=2):
    ...
    for attempt in range(max_retries):
        try:
            ...
            resp = safe_request("post", f"{MESHY_API_URL}/text-to-3d", headers=headers, json=payload, timeout=30)
            data = resp.json()
            ...
            for _ in range(30):
                time.sleep(5)
                resp = safe_request("get", f"{MESHY_API_URL}/text-to-3d/{task_id}", headers=headers, timeout=30)
                data = resp.json()
                ...
                if status == "SUCCEEDED":
                    ...
                    resp = safe_request("post", f"{MESHY_API_URL}/text-to-3d", headers=headers, json=refine_payload, timeout=30)
                    ...
                    for _ in range(30):
                        time.sleep(5)
                        resp = safe_request("get", f"{MESHY_API_URL}/text-to-3d/{refine_task_id}", headers=headers, timeout=30)
                        refine_data = resp.json()
                        ...
                        if refine_status == "SUCCEEDED":
                            ...
                            print(f"Meshy STL URL: {model_url[:80]}...")
                            model_resp = safe_request("get", model_url, timeout=90)
                            if model_resp.status_code == 200:
                                return model_resp.content
                            else:
...
def process_text_generation(chat_id, user_id, prompt, gen_type):
    ...
    try:
        if service == "meshy" and MESHY_API_KEY:
            model = meshy_generate_with_retry(prompt)
        else:
        ...
        if model:
            ...
            caption = f"✅ Модель готова! ({'Meshy' if service == 'meshy' else 'Hunyuan'})\n📝 {prompt[:100]}"
            if gen_type == "free" and user_id != ADMIN_CHAT_ID:
                caption += f"\n🎁 Осталось бесплатно: {remaining} из {FREE_LIMIT}"
...
def handle_help_info(chat_id):
    text = (
        ...
        f"🎁 *Бесплатно:* {FREE_LIMIT} модели на первый раз\n"
...
def handle_balance(chat_id, user_id):
    ...
    if is_premium:
        ...
        status = f"🎁 Бесплатно осталось: {remaining} из {FREE_LIMIT}"
    
    text = f"💰 *Ваш баланс:*\n\n{status}\n⚡ Сервис: {'Meshy' if service == 'meshy' else 'Hunyuan'}"
    send_message(chat_id, text, keyboard=back_keyboard)
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                for update in updates:
                    ...
                    print(f"!!! [DEBUG] Тип обновления: {list(update.keys())}", flush=True)
                    ...
                    if "callback_query" in update:
                        ...
                        if data == "menu":
                            ...
                            send_message(chat_id, f"⚡ Текущий сервис: {'Meshy' if current == 'meshy' else 'Hunyuan'}\nВыберите новый:", keyboard=service_keyboard)
                        elif data == "set_hunyuan":
                            user_service_choice[user_id] = "hunyuan"
                            send_message(chat_id, "✅ Выбран сервис: Hunyuan", keyboard=main_keyboard)
                        elif data == "set_meshy":
                            if MESHY_API_KEY:
                                user_service_choice[user_id] = "meshy"
                                send_message(chat_id, "✅ Выбран сервис: Meshy", keyboard=main_keyboard)
                            else:
                                send_message(chat_id, "⚠️ Meshy временно недоступен.", keyboard=main_keyboard)
                        elif data == "buy_one":
                    ...
                    if "message" in update:
                        ...
                        if chat_id in user_states:
                            ...
                            if state == "awaiting_text" and "text" in msg:
                                ...
                                if not check_content(prompt):
                                    send_message(chat_id, "⚠️ Ваш запрос содержит запрещённые слова. Пожалуйста, измените описание.", keyboard=main_keyboard)
                                    continue
                                ...
                                if user_busy.get(user_id, False):
                                    send_message(chat_id, "⏳ У вас уже выполняется генерация.", keyboard=main_keyboard)
                                    continue
                                ...
                                service = user_service_choice.get(user_id, "hunyuan")
                                send_message(chat_id, f"⏳ Генерирую модель ({'Meshy' if service == 'meshy' else 'Hunyuan'})... (1-4 мин)")
                                ...
                                can, gen_type = can_generate(user_id)
                                if not can:
                                    send_message(chat_id, "❌ Бесплатные генерации закончились.", keyboard=main_keyboard)
                                    continue
                        ...
                        # === Обычное сообщение → пересылка админу (ИСПРАВЛЕНО) ===
                        if "text" in msg and msg["text"] and not msg["text"].startswith('/') and user_id != ADMIN_CHAT_ID:
                            print(f"!!! [DEBUG] Вызов forward_to_admin для user_id={user_id}", flush=True)
                            username = msg["from"].get("username", None)
                            first_name = msg["from"].get("first_name", "")
                            forward_to_admin(user_id, username, first_name, msg["text"])
                            send_message(chat_id, "📨 Спасибо за сообщение! Я передал его разработчику.")
                            continue
