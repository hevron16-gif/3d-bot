...
def hunyuan_generate_from_text(prompt, retry=2):
    ...
    for attempt in range(retry):
        try:
            ...
            resp = requests.post(f"https://{HUNYUAN_HOST}", headers=headers, data=payload, timeout=(60 if attempt==0 else 90))
            data = resp.json()
            ...
            for _ in range(60):  # увеличил количество попыток до 60 (~6 минут)
                time.sleep(6)
            ...
            # Если задача не завершилась за отведённое время, пробуем ещё раз
            if attempt < retry - 1:
...
def hunyuan_generate_from_photo(image_base64, retry=2):
    for attempt in range(retry):
        try:
            action = "SubmitHunyuanTo3DProJob"
