...
# ========== ДЛЯ ОТВЕТОВ АДМИНА ==========
pending_reply = {}  # admin_chat_id -> user_id (кому отвечать)
admin_reply_mode = {}  # admin_chat_id -> True (админ в режиме ответа)
...
def reply_to_user(admin_message_text):
    """Отправляет ответ пользователю от имени бота"""
            ...
            if resp.status_code == 200:
                for update in resp.json().get("result", []):
                    ...
                    if "message" in update:
                        ...
                        # Команда /cancelreply (админ отменяет ответ)
                        ...
                        # === ВАЖНО: Пользователь в режиме ожидания генерации ===
                        if chat_id in user_states:
                            state = user_states[chat_id]
                            
                            # Ждём текст
                            if state == "awaiting_text" and "text" in msg:
                                # Но сначала проверим — это не команда и не случайное сообщение
                                prompt = msg["text"]
                                if prompt.startswith('/'):
                                    continue  # Пропускаем команды
                            ...
                            # Пользователь в режиме ожидания, но шлёт что-то другое
                        ...
                        # === Обычное сообщение не в режиме генерации ===
