...
ADMIN_CHAT_ID = 703281454


if not TELEGRAM_BOT_TOKEN or not TENCENT_SECRET_ID or not TENCENT_SECRET_KEY:
...
def forward_to_admin(update, text):
    if ADMIN_CHAT_ID:
        try:
...
user_states = {}  # {chat_id: "awaiting_text" or "awaiting_photo"}
...
def hunyuan_generate_from_text(prompt):
    ...
    for attempt in range(3):
        try:
            ...
            resp = requests.post(f"https://{HUNYUAN_HOST}", headers=headers, data=payload, timeout=120)
            data = resp.json()
...
def handle_help_info(chat_id):
    text = (
        ...
        "2️⃣ Выберите способ (текст или фото)\n"
                ...
                for update in resp.json().get("result", []):
                    ...
                    if "message" in update and "successful_payment" in update["message"]:
                        ...
                        if payload == "single_generation":
                            ...
                            send_message(user_id, "✅ Разовая генерация активирована! Теперь нажмите «Сгенерировать модель».")
                            ...
                            send_message(user_id, "✅ Подписка на месяц активирована! Теперь нажмите «Сгенерировать модель».")
                    ...
                    if "callback_query" in update:
                        ...
                        if data == "menu":
                            ...
                            send_message(chat_id, "❌ Действие отменено. Выберите действие в меню:", keyboard=main_keyboard)
                        ...
                        # Обработка текста, если бот ждёт промт
                        if chat_id in user_states and user_states[chat_id] == "awaiting_text" and "text" in msg:
                            ...
                            try:
                                ...
                                send_alert(f"❌ Ошибка генерации текста у {user_id}: {str(e)}")
                        
                        # Обработка фото, если бот ждёт фото
                            ...
                            try:
                                ...
                                send_message(chat_id, f"❌ Ошибка: {str(e)}")
                                send_alert(f"❌ Ошибка генерации фото у {user_id}: {str(e)}")
                        
                        # Обычные сообщения не во время генерации — пересылаем админу
                        elif "text" in msg and msg["text"] and not msg["text"].startswith('/'):
                            forward_to_admin(update, msg["text"])
                            send_message(chat_id, "📨 Спасибо за сообщение! Разработчик скоро ответит вам.")
