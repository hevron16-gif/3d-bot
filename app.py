...
def hunyuan_generate_from_blueprint(image_base64, output_format="stl", max_retries=3):
    """Генерация по чертежу (только ImageBase64, без Prompt)"""
    for attempt in range(max_retries):
        try:
...
def process_blueprint_generation(chat_id, user_id, image_base64, gen_type):
    """Генерация по чертежу (только фото, без промта)"""
    output_format = user_format_choice.get(user_id, "stl")
    try:
        model, fmt = hunyuan_generate_from_blueprint(image_base64, output_format)
        if model:
            use_generation(user_id, gen_type)
            update_stats(user_id)
            remaining = max(0, FREE_LIMIT - user_free_used.get(user_id, 0))
            filename = f"model.{fmt}"
            caption = t(user_id, "model_ready", service="HUNYUAN", fmt=fmt.upper(), prompt="Чертёж")
            if gen_type == "free" and user_id != ADMIN_CHAT_ID:
                caption += t(user_id, "free_remaining", remaining=remaining, limit=FREE_LIMIT)
            send_document(chat_id, model, filename=filename, caption=caption)
        else:
