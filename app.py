...
# ========== ДЛЯ ЧЕРТЕЖЕЙ (MULTI-VIEW) ==========
user_blueprint_images = {}  # {user_id: [список file_id]}
user_blueprint_prompt = {}  # {user_id: "промт"}
...
TEXTS = {
    "ru": {
        ...
        "choose_service": "⚡ Текущий сервис: {current}\nВыберите новый:",
        ...
        "blueprint_start": "📐 *Генерация по чертежу*\n\nЗагрузите 3-8 фото объекта с разных ракурсов:\n1. Вид спереди\n2. Вид сбоку\n3. Вид сверху\n\nПосле загрузки нажмите «Готово».",
        "blueprint_photo_added": "📸 Фото {count} добавлено. Загрузите ещё или нажмите «Готово».",
        "blueprint_prompt": "📝 Напишите короткое описание объекта (материал, размер).",
        "blueprint_ready": "📐 Загружено {count} фото. Начинаю генерацию...",
    },
    "en": {
        ...
        "choose_service": "⚡ Current service: {current}\nChoose a new one:",
        ...
        "blueprint_start": "📐 *Blueprint Generation*\n\nUpload 3-8 photos of the object from different angles:\n1. Front view\n2. Side view\n3. Top view\n\nPress «Done» when ready.",
        "blueprint_photo_added": "📸 Photo {count} added. Upload more or press «Done».",
        "blueprint_prompt": "📝 Write a short description of the object (material, size).",
        "blueprint_ready": "📐 {count} photos uploaded. Starting generation...",
    }
}
...
def hunyuan_generate_with_retry(prompt, output_format="stl", max_retries=3):
    """Генерация по тексту (улучшенная: Model 3.1, PBR, высокий FaceCount)"""
    for attempt in range(max_retries):
        try:
            print(f"Hunyuan: попытка {attempt+1}")
            ...
            payload_dict = {
                ...
                "GenerateType": "Normal",
                "FaceCount": 1000000,
                "ResultFormat": output_format.upper()
            }
                ...
                if "Response" in data:
                    ...
                    if status == "DONE":
                        ...
                        if result_files and "Url" in result_files[0]:
                            model_url = result_files[0]["Url"]
                            if output_format == "glb":
                                model_resp = safe_request("get", model_url, timeout=120)
                                if model_resp.status_code == 200:
                                    return model_resp.content, "glb"
                            # Конвертируем в STL
                            conv_action = "Convert3DFormat"
                            conv_payload = json.dumps({"File3D": model_url, "Format": "STL"})
                            ...
                            if "Response" in conv_data and "ResultFile3D" in conv_data["Response"]:
                                ...
                                model_resp = safe_request("get", stl_url, timeout=120)
                                if model_resp.status_code == 200:
                                    return model_resp.content, "stl"
                    elif status == "FAIL":
...
def hunyuan_generate_from_views(image_urls_list, prompt, output_format="stl", max_retries=3):
    """Генерация по чертежам (MultiView) — улучшенная версия"""
    for attempt in range(max_retries):
        try:
            print(f"Hunyuan MultiView: попытка {attempt+1}, фото: {len(image_urls_list)}")
            action = "SubmitHunyuanTo3DProJob"
            payload_dict = {
                "Model": "3.1",
                "Prompt": prompt[:900],
                "MultiViewImages": image_urls_list,
                "EnablePBR": True,
                "GenerateType": "Normal",
                "FaceCount": 1000000,
                "ResultFormat": output_format.upper()
            ...
            print(f"Hunyuan MultiView ошибка в попытке {attempt+1}: {e}")
            if attempt == max_retries - 1:
                raise e
            time.sleep(15)
    raise Exception("Hunyuan MultiView: не удалось сгенерировать модель")
        ...
        try:
            ...
            for _ in range(40):
                ...
                if status == "SUCCEEDED":
                    ...
                    print(f"Meshy refine task ID: {refine_task_id}")
                    for _ in range(40):
                        time.sleep(5)
                        ...
                        print(f"Meshy refine статус: {refine_status}")
                        if refine_status == "SUCCEEDED":
                            ...
                            print(f"Meshy URL: {model_url[:80]}...")
                            model_resp = safe_request("get", model_url, timeout=120)
                            if model_resp.status_code == 200:
    ...
    try:
        ...
        if service == "meshy" and MESHY_API_KEY:
            ...
            try:
                ...
                if MESHY_API_KEY:
                    print(f"Hunyuan упал, переключаем на Meshy: {hunyuan_error}")
                    model, fmt = meshy_generate_with_retry(prompt, output_format)
                    used_service = "meshy"
                else:
                    raise hunyuan_error
...
def process_blueprint_generation(chat_id, user_id, gen_type):
    """Генерация по чертежам (MultiView)"""
    image_urls = []
    for file_id in user_blueprint_images.get(user_id, []):
        url = get_image_url(file_id)
        if url:
            # Определяем view по порядку (упрощённо)
            views = ["front", "left", "top", "back", "right", "bottom"]
            idx = len(image_urls)
            view = views[idx] if idx < len(views) else f"view_{idx}"
            image_urls.append({"View": view, "ImageUrl": url})
    
    prompt = user_blueprint_prompt.get(user_id, "3D object")
    output_format = user_format_choice.get(user_id, "stl")
    
    try:
        model, fmt = hunyuan_generate_from_views(image_urls, prompt, output_format)
        if model:
            use_generation(user_id, gen_type)
            update_stats(user_id)
            remaining = max(0, FREE_LIMIT - user_free_used.get(user_id, 0))
            filename = f"model.{fmt}"
            caption = t(user_id, "model_ready", service="HUNYUAN", fmt=fmt.upper(), prompt=prompt[:100])
            if gen_type == "free" and user_id != ADMIN_CHAT_ID:
                caption += t(user_id, "free_remaining", remaining=remaining, limit=FREE_LIMIT)
            send_document(chat_id, model, filename=filename, caption=caption)
        else:
        ...
        send_message(chat_id, t(user_id, "error", error=str(e)))
        send_alert(f"Ошибка генерации по чертежу у {user_id}: {str(e)}")
    finally:
        user_blueprint_images.pop(user_id, None)
        user_blueprint_prompt.pop(user_id, None)
        user_busy[user_id] = False
...
def process_photo_generation(chat_id, user_id, image_base64, gen_type):
    try:
        model, fmt = hunyuan_generate_from_photo(image_base64)
        if model:
            use_generation(user_id, gen_type)
            update_stats(user_id)
            caption = "✅ Модель по фото готова!"
            if gen_type == "free" and user_id != ADMIN_CHAT_ID:
                remaining = max(0, FREE_LIMIT - user_free_used.get(user_id, 0))
                caption += t(user_id, "free_remaining", remaining=remaining, limit=FREE_LIMIT)
            send_document(chat_id, model, filename="model.stl", caption=caption)
        else:
            send_message(chat_id, t(user_id, "error_generation"))
    except Exception as e:
        send_message(chat_id, t(user_id, "error", error=str(e)))
...
def hunyuan_generate_from_photo(image_base64, max_retries=3):
    for attempt in range(max_retries):
        try:
            print(f"Hunyuan фото: попытка {attempt+1}")
            action = "SubmitHunyuanTo3DProJob"
            payload_dict = {
                "Model": "3.1",
...
def get_main_keyboard(user_id):
    ...
    return {
        "inline_keyboard": [
            ...
            [{"text": "📐 Генерация по чертежу" if lang == "ru" else "📐 Blueprint Generation", "callback_data": "gen_blueprint"}],
            [{"text": "⚡ Выбрать сервис" if lang == "ru" else "⚡ Choose Service", "callback_data": "choose_service"}],
...
def get_blueprint_keyboard(user_id):
    lang = user_language.get(user_id, "ru")
    return {
        "inline_keyboard": [
            [{"text": "✅ Готово" if lang == "ru" else "✅ Done", "callback_data": "blueprint_done"}],
            [{"text": "❌ Отмена" if lang == "ru" else "❌ Cancel", "callback_data": "cancel"}],
        ]
    }
...
service_keyboard = {
    "inline_keyboard": [
        [{"text": "🔵 Hunyuan (по умолчанию)", "callback_data": "set_hunyuan"}],
        [{"text": "🟣 Meshy (если доступен)", "callback_data": "set_meshy"}],
...
def handle_balance(chat_id, user_id):
    ...
    text = t(user_id, "balance", status=status, service=("Meshy" if service == "meshy" else "Hunyuan"), fmt=fmt.upper())
    send_message(chat_id, text, keyboard=back_keyboard)
                ...
                for update in updates:
                    ...
                    if "callback_query" in update:
                        ...
                        if data == "menu":
                            ...
                            current = user_service_choice.get(user_id, "hunyuan")
                            send_message(chat_id, t(user_id, "choose_service", current=("Meshy" if current == "meshy" else "Hunyuan")), keyboard=service_keyboard)
                            ...
                            send_message(chat_id, t(user_id, "lang_set"), keyboard=get_main_keyboard(user_id))
                        elif data == "gen_blueprint":
                            user_states[chat_id] = "awaiting_blueprint"
                            user_blueprint_images[user_id] = []
                            send_message(chat_id, t(user_id, "blueprint_start"), keyboard=get_blueprint_keyboard(user_id))
                        elif data == "blueprint_done":
                            images = user_blueprint_images.get(user_id, [])
                            if len(images) < 3:
                                send_message(chat_id, "⚠️ Нужно минимум 3 фото. Загрузите ещё.", keyboard=get_blueprint_keyboard(user_id))
                            else:
                                user_states[chat_id] = "awaiting_blueprint_prompt"
                                send_message(chat_id, t(user_id, "blueprint_prompt"), keyboard=cancel_keyboard)
                        elif data == "buy_one":
                            ...
                            service = user_service_choice.get(user_id, "hunyuan")
                            fmt = user_format_choice.get(user_id, "stl")
                            send_message(chat_id, t(user_id, "send_text", service=("Meshy" if service == "meshy" else "Hunyuan"), fmt=fmt.upper()), keyboard=cancel_keyboard)
                        elif data == "gen_photo_confirm":
                            user_states[chat_id] = "awaiting_photo"
                            send_message(chat_id, t(user_id, "send_photo"), keyboard=cancel_keyboard)
                        elif data == "cancel":
                    ...
                    if "message" in update:
                        ...
                        if chat_id in user_states:
                            ...
                            # Приём фото для чертежа
                            if state == "awaiting_blueprint" and "photo" in msg:
                                file_id = msg["photo"][-1]["file_id"]
                                if user_id not in user_blueprint_images:
                                    user_blueprint_images[user_id] = []
                                user_blueprint_images[user_id].append(file_id)
                                count = len(user_blueprint_images[user_id])
                                send_message(chat_id, t(user_id, "blueprint_photo_added", count=count), keyboard=get_blueprint_keyboard(user_id))
                                continue
                            
                            # Приём описания для чертежа
                            if state == "awaiting_blueprint_prompt" and "text" in msg:
                                prompt = msg["text"]
                                if prompt.startswith('/'):
                                    continue
                                user_blueprint_prompt[user_id] = prompt
                                del user_states[chat_id]
                                images = user_blueprint_images.get(user_id, [])
                                ...
                                send_message(chat_id, t(user_id, "blueprint_ready", count=len(images)))
                                executor.submit(process_blueprint_generation, chat_id, user_id, gen_type)
                                continue
                            ...
                            if state == "awaiting_text" and "text" in msg:
                                ...
                                service = user_service_choice.get(user_id, "hunyuan")
                                fmt = user_format_choice.get(user_id, "stl")
                                send_message(chat_id, t(user_id, "generating", service=("Meshy" if service == "meshy" else "Hunyuan"), fmt=fmt.upper()))
