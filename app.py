...
# ========== БЛОКИРОВКА ПОЛЬЗОВАТЕЛЯ НА ВРЕМЯ ГЕНЕРАЦИИ ==========
user_busy = {}  # user_id -> True если идёт генерация
...
def forward_to_admin(user_id, username, first_name, text):
    ...
    try:
        ...
        print("🟢 [DEBUG] Сообщение админу отправлено")
...
# ========== ФОНОВАЯ ГЕНЕРАЦИЯ (в отдельном потоке) ==========
def process_text_generation(chat_id, user_id, prompt, gen_type):
    """Запускается в отдельном потоке — не блокирует других пользователей"""
    try:
        ...
        user_busy[user_id] = False  # Освобождаем пользователя
            ...
            if resp.status_code == 200:
                for update in resp.json().get("result", []):
                    ...
                    if "message" in update:
                        ...
                        # Пересылка обычных сообщений админу
                        ...
                        if chat_id in user_states and user_states[chat_id] == "awaiting_text" and "text" in msg:
                            ...
                            # Проверка — не занят ли уже пользователь
                            ...
                            user_busy[user_id] = True  # Блокируем пользователя
                        ...
                        # Генерация по фото
                            ...
                            # Получаем фото и запускаем в ОТДЕЛЬНОМ ПОТОКЕ
