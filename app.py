...
def generate_3d_model(prompt=None, image_base64=None, image_caption=None):
    ...
    if image_base64:
        ...
        print(f"[TEXT] Генерация по тексту: {prompt[:100]}...")
    ...
    print(f"[DEBUG] Отправляем запрос на {endpoint}")
    ...
    while time.time() - start_time < 420:
        ...
        if status_resp.status_code == 200:
            ...
            if status == "SUCCEEDED":
                ...
                raise Exception("Meshy вернул FAILED")
        ...
        if "text" in msg:
            ...
            if text == "/start":
                requests.post(API_URL + "/sendMessage", json={
                    "chat_id": chat_id,
                    "text": "👋 Бот работает!\nОтправь описание модели или фото.",
                    "reply_markup": {
                        "inline_keyboard": [
                ...
                try:
                    ...
                    requests.post(API_URL + "/sendMessage", json={"chat_id": chat_id, "text": f"❌ Ошибка: {str(e)[:250]}"})

        elif "photo" in msg:
            requests.post(API_URL + "/sendMessage", json={"chat_id": chat_id, "text": "⏳ Получил фото, обрабатываю..."})
