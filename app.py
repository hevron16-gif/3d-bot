...
TEXTS = {
    "ru": {
        ...
        "choose_format": "📦 Текущий формат: {fmt}\n\n• STL — для 3D-печати (без цвета)\n• GLB — для игр и редакторов (с цветом, можно разделить на части)\n\nВыберите формат:",
        "format_set": "✅ Выбран формат: {fmt_name}",
        "format_set_glb": "✅ Выбран GLB — этот формат можно разделить на части!",
        "format_set_stl": "✅ Выбран STL — для 3D-печати. Разделение недоступно.",
        ...
        "part_job_send": "📤 Отправьте файл модели (GLB, OBJ, FBX).\n\n⚠️ STL не поддерживается — сначала сгенерируйте в GLB!\n⚠️ До 50 МБ через Telegram.\n📎 Или отправьте ссылку до 200 МБ.",
        "part_job_link_send": "🔗 Отправьте прямую ссылку на файл (GLB, OBJ, FBX) до 200 МБ.\n\n⚠️ STL не поддерживается!\nСсылка должна быть прямой (заканчиваться на .glb, .obj, .fbx).",
        ...
        "wrong_format": "❌ Неподдерживаемый формат.\n\nОтправьте GLB, OBJ или FBX.",
        "stl_not_supported": "❌ STL нельзя разделить на части.\n\nСгенерируйте модель в формате GLB (кнопка «📦 Формат» → GLB) или загрузите GLB/OBJ/FBX файл.",
    ...
    "en": {
        ...
        "format_set_glb": "✅ GLB selected — this format can be split into parts!",
        "format_set_stl": "✅ STL selected — for 3D printing. Split not available.",
        ...
        "part_job_send": "📤 Send a model file (GLB, OBJ, FBX).\n\n⚠️ STL not supported — generate in GLB first!\n⚠️ Up to 50 MB via Telegram.\n📎 Or send a link up to 200 MB.",
        "part_job_link_send": "🔗 Send a direct link to the file (GLB, OBJ, FBX) up to 200 MB.\n\n⚠️ STL not supported!\nLink must be direct (ending in .glb, .obj, .fbx).",
        ...
        "stl_not_supported": "❌ STL cannot be split into parts.\n\nGenerate a model in GLB format («📦 Format» → GLB) or upload a GLB/OBJ/FBX file.",
...
def send_document(chat_id, file_content, filename="model.stl", caption=""):
    ...
    requests.post(API_URL + "/sendDocument", files=files, data=data, timeout=180)
...
def safe_request(method, url, max_retries=3, timeout=600, **kwargs):
...
# ========== КОНВЕРТАЦИЯ В FBX ==========
def convert_to_fbx(file_url, max_retries=3):
    """Конвертация GLB/OBJ/FBX в FBX через Convert3DFormat.
    Таймаут 600 секунд (10 минут) — Tencent Cloud может обрабатывать до 9 минут.
    """
    for attempt in range(max_retries):
        try:
            print(f"🔄 Конвертация FBX, попытка {attempt+1}/{max_retries}...")
            action = "Convert3DFormat"
            payload = json.dumps({"File3D": file_url, "Format": "FBX"})
            headers = get_tencent_headers(action, payload)
            resp = safe_request("post", f"https://{HUNYUAN_HOST}", headers=headers, data=payload, timeout=600)
            data = resp.json()
            
            print(f"📦 Ответ Convert3DFormat: {json.dumps(data, ensure_ascii=False)[:300]}")


            if "Response" in data and "ResultFile3D" in data["Response"]:
                fbx_url = data["Response"]["ResultFile3D"]
                print(f"✅ Конвертирован в FBX: {fbx_url[:80]}...")
                return fbx_url
            else:
            ...
            print(f"❌ Конвертация FBX, попытка {attempt+1}/{max_retries} ошибка: {e}")
            if attempt == max_retries - 1:
                raise e
            time.sleep(15)


    raise Exception("Не удалось конвертировать в FBX после всех попыток")
                ...
                if "Response" in data:
                    ...
                    if status == "DONE":
                        ...
                        if result_files and "Url" in result_files[0]:
                            ...
                            glb_resp = safe_request("get", glb_url, timeout=300)
                            if glb_resp.status_code != 200:
                                raise Exception("Не удалось скачать GLB")
                            glb_data = glb_resp.content


                            print(f"Hunyuan: возвращаю {output_format.upper()}, размер: {len(glb_data)} байт")
                            return glb_data, output_format
                    elif status == "FAIL":
                ...
                if "Response" in data:
                    ...
                    if status == "DONE":
                        ...
                        if result_files and "Url" in result_files[0]:
                            ...
                            print(f"Hunyuan фото: возвращаю {output_format.upper()}, размер: {len(glb_data)} байт")
                            return glb_data, output_format
                    elif status == "FAIL":
                        raise Exception("Generation FAILED")
...
def submit_part_job(fbx_url, max_retries=2):
    for attempt in range(max_retries):
        try:
            ...
            print(f"📦 Ответ PartJob: {json.dumps(data, ensure_ascii=False)[:300]}")
            ...
            if "Response" in data and "JobId" in data["Response"]:
                ...
                raise Exception(f"PartJob submit error: {error_msg}")
...
def download_and_send_parts(chat_id, result_files):
    ...
    for i, part in enumerate(result_files):
        ...
        try:
            file_data = safe_request("get", file_url, timeout=300).content
            if file_type == "IMAGE":
                requests.post(API_URL + "/sendPhoto", data={
                    "chat_id": chat_id,
                    "caption": t(chat_id, "part_job_schema")
                }, files={"photo": ("schema.jpg", file_data)}, timeout=60)
            else:
                obj_count += 1
                ...
                if status == "SUCCEEDED":
                    ...
                    for _ in range(40):
                        ...
                        if refine_status == "SUCCEEDED":
                            ...
                            model_resp = safe_request("get", model_url, timeout=300)
...
def process_part_job(chat_id, user_id, file_url, gen_type):
    try:
        # Шаг 1: Конвертируем в FBX
        send_message(chat_id, "🔄 Конвертирую в FBX...")
        fbx_url = convert_to_fbx(file_url)


        # Шаг 2: Отправляем на разделение
        send_message(chat_id, "🧩 Разделяю модель на части... (1-3 мин)")
                ...
                for update in updates:
                    ...
                    if "callback_query" in update:
                        ...
                        if data == "menu":
                            ...
                            send_message(chat_id, t(user_id, "format_set_stl"), keyboard=get_main_keyboard(user_id))
                        elif data == "set_glb":
                            user_format_choice[user_id] = "glb"
                            send_message(chat_id, t(user_id, "format_set_glb"), keyboard=get_main_keyboard(user_id))
                            ...
                            send_invoice(chat_id,
                                        ...
                                        "Одна генерация (текст/фото)" if user_language.get(user_id, "ru") == "ru" else "One generation (text/photo)",
                    ...
                    if "message" in update:
                        ...
                        if "text" in msg and msg["text"] == "/start":
                            ...
                            text = t(user_id, "start", remain=remain, limit=FREE_LIMIT, part_remain=part_remain, part_limit=FREE_PART_LIMIT)
                        ...
                        if chat_id in user_states:
                            ...
                            if state == "awaiting_text" and "text" in msg:
                                ...
                                if file_name.endswith(".stl"):
                                    send_message(chat_id, t(user_id, "stl_not_supported"), keyboard=get_main_keyboard(user_id))
                                    continue


                                if not any(file_name.endswith(ext) for ext in [".glb", ".obj", ".fbx"]):
                                ...
                                if link.lower().endswith(".stl"):
                                    send_message(chat_id, t(user_id, "stl_not_supported"), keyboard=get_main_keyboard(user_id))
                                    continue


                                if not any(link.lower().endswith(ext) for ext in [".glb", ".obj", ".fbx"]):
                                    send_message(chat_id, t(user_id, "wrong_format"), keyboard=get_main_keyboard(user_id))
                                    continue
