...
def create_text_task(prompt):
    ...
    # Получаем полный URL для проверки статуса ИЗ ОТВЕТА API
...
def wait_for_task(task_id, task_type="text"):
    ...
    # Используем правильный эндпоинт как в документации
    if task_type == "text":
        endpoint = f"{MESHY_API_URL}/text-to-3d/{task_id}"
    else:
        endpoint = f"{MESHY_API_URL}/image-to-3d/{task_id}"
                ...
                for update in resp.json().get("result", []):
                    ...
                    if "callback_query" in update:
                        ...
                        if "text" in msg and msg["text"] == "/start":
                            ...
                            try:
                                task_id, result_url = create_text_task(msg["text"])
                            ...
                            try:
                                ...
                                task_id, result_url = create_photo_task(file_url)
                                print(f"🟢 Task ID: {task_id}")
