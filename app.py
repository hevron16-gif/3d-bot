...
admin_reply_mode = {}
user_service_choice = {}
user_format_choice = {}  # "stl" или "glb"
...
# ========== ФОНОВАЯ ГЕНЕРАЦИЯ (С АВТО-ПЕРЕКЛЮЧЕНИЕМ) ==========
def process_text_generation(chat_id, user_id, prompt, gen_type):
    ...
    try:
        ...
        if model:
            ...
            caption = f"✅ Модель готова! ({used_service.upper()}, {fmt.upper()})\n📝 {prompt[:100]}"
...
main_keyboard = {
    "inline_keyboard": [
        ...
        [{"text": "📦 Формат: STL", "callback_data": "choose_format"}],
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                for update in updates:
                    ...
                    # === НОВОЕ: Подтверждение pre_checkout_query ===
                    ...
                    if "callback_query" in update:
                        ...
                        if data == "menu":
                            ...
                            fmt = user_format_choice.get(user_id, "stl")
                            main_keyboard["inline_keyboard"][3] = [{"text": f"📦 Формат: {fmt.upper()}", "callback_data": "choose_format"}]
                            ...
                            send_message(chat_id, f"📦 Текущий формат: {current_fmt.upper()}\n\nSTL — для 3D-печати\nGLB — для игр и 3D-редакторов\n\nВыберите формат:", keyboard=format_keyboard)
                        elif data == "set_stl":
                            user_format_choice[user_id] = "stl"
                            send_message(chat_id, "✅ Выбран формат: STL (для 3D-печати)", keyboard=main_keyboard)
                        elif data == "set_glb":
                            user_format_choice[user_id] = "glb"
                            send_message(chat_id, "✅ Выбран формат: GLB (для игр и редакторов)", keyboard=main_keyboard)
                        elif data == "buy_one":
                        ...
                        if chat_id in user_states:
                            ...
                            if state == "awaiting_text" and "text" in msg:
                                ...
                                if not check_content(prompt):
                                    send_message(chat_id, "⚠️ Ваш запрос содержит запрещённые слова.", keyboard=main_keyboard)
                                    continue
                                ...
                                service = user_service_choice.get(user_id, "hunyuan")
                                fmt = user_format_choice.get(user_id, "stl")
                                send_message(chat_id, f"⏳ Генерирую модель ({'Meshy' if service == 'meshy' else 'Hunyuan'}, {fmt.upper()})... (1-4 мин)")
