...
# Soft-fail: Render health-check must not crash if env not set yet
_missing = []
if not TELEGRAM_BOT_TOKEN:
    _missing.append("TELEGRAM_BOT_TOKEN")
if not TENCENT_SECRET_ID or not TENCENT_SECRET_KEY:
    _missing.append("TENCENT_SECRET_ID/TENCENT_SECRET_KEY")
if _missing:
    print(f"⚠️ [DEBUG] Missing env (bot idle until set): {', '.join(_missing)}", flush=True)
if MESHY_API_KEY:
    print("🟢 [DEBUG] Meshy API Key: set", flush=True)
else:
    print("⚠️ [DEBUG] Meshy API Key: NOT SET", flush=True)


API_URL = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}" if TELEGRAM_BOT_TOKEN else ""
HUNYUAN_HOST = "hunyuan.intl.tencentcloudapi.com"
MESHY_API_URL = "https://api.meshy.ai/v2"

# ── AI3D API (новый хост для Convert3DFormat и Part API) ──
...
TEXTS = {
    "ru": {
        ...
        "drawing_start": "📐 Отправьте чертёж или эскиз.\n\n📝 Добавьте текстовое описание.",
        "drawing_generating": "⏳ Генерирую модель по чертежу (3.0, Sketch)... 2-4 мин.",
        "part_split_start": "🧩 Разбиваю модель на части...\n\nЭтот процесс занимает 2-4 минуты.",
        "part_split_no_model": "⚠️ Нет последней модели для разбивки.",
        "part_split_done": "✅ Разбивка завершена! {count} частей.",
        ...
        "part_converting": "🧩 Конвертирую модель в FBX для разбивки...\n⏳ Это займёт 1-2 минуты.",
        "part_splitting": "🧩 Разбиваю модель на части...\n⏳ Это займёт 2-4 минуты.",
        "part_fallback": "⚠️ Конвертация FBX не удалась.\n🔄 Пробую обходной путь: регенерирую модель в формате FBX...\n⏳ Это займёт 3-4 минуты.",
    },
    "en": {
        ...
        "drawing_start": "📐 Send a drawing or sketch.\n\n📝 Add a text description.",
        "drawing_generating": "⏳ Generating model from drawing (3.0, Sketch)... 2-4 min.",
        "part_split_start": "🧩 Splitting the model into parts...\n\nThis takes 2-4 minutes.",
        "part_split_no_model": "⚠️ No previous model to split.",
        "part_split_done": "✅ Split complete! {count} parts.",
        ...
        "part_converting": "🧩 Converting model to FBX for splitting...\n⏳ This takes 1-2 minutes.",
        "part_splitting": "🧩 Splitting model into parts...\n⏳ This takes 2-4 minutes.",
        "part_fallback": "⚠️ FBX conversion failed.\n🔄 Trying fallback: regenerating model in FBX format...\n⏳ This takes 3-4 minutes.",
    }
}
...
user_last_hunyuan_job = {}  # user_id -> JobId (для перезапроса URL)
user_last_image = {}  # user_id -> image_base64 (для fallback-регенерации)
...
# ── AI3D-версия подписи (для Convert3DFormat и Part API) ──
def get_ai3d_headers(action, payload):
    """Полная копия get_tencent_headers, но с service='ai3d',
    host='ai3d.tencentcloudapi.com', version='2025-05-13'."""
    service = AI3D_SERVICE
    host = AI3D_HOST
    region = AI3D_REGION
    version = AI3D_VERSION
    algorithm = "TC3-HMAC-SHA256"
    timestamp = int(time.time())
    date = datetime.utcfromtimestamp(timestamp).strftime("%Y-%m-%d")
    ct = "application/json"
                ...
                if "Response" in data:
                    ...
                    if status == "DONE":
                        ...
                        if result_files and "Url" in result_files[0]:
                            ...
                            # ★ Сохраняем GLB URL и JobId для PartSplit ★
                            if user_id:
                                user_last_glb_url[user_id] = glb_url
                                user_last_hunyuan_job[user_id] = job_id
                ...
                if "Response" in data:
                    ...
                    if status == "DONE":
                        ...
                        if result_files and "Url" in result_files[0]:
                            ...
                            # ★ Сохраняем GLB URL, JobId и image для PartSplit ★
                            if user_id:
                                user_last_glb_url[user_id] = glb_url
                                user_last_hunyuan_job[user_id] = job_id
                                user_last_image[user_id] = image_base64
...
# ========== ГЕНЕРАЦИЯ ПО ЧЕРТЕЖАМ (Sketch, Model 3.0) ==========
def hunyuan_generate_from_drawing(prompt, image_base64=None, max_retries=3, user_id=None):
    """Генерация по чертежам/скетчам через Hunyuan.
    ВАЖНО: использует Model '3.0' (а не '3.1'), т.к. GenerateType='Sketch'
    не поддерживается в версии 3.1.
    Возвращает (data, format) — GLB или STL в зависимости от output_format."""
    for attempt in range(max_retries):
        try:
            print(f"Hunyuan Sketch: попытка {attempt+1}")
            action = "SubmitHunyuanTo3DProJob"
            payload_dict = {
                "Model": "3.0",  # ★ Sketch поддерживается только в 3.0 ★
                "Prompt": prompt[:900],
                "EnablePBR": True,
                "GenerateType": "Sketch",
                "FaceCount": 1000000
            ...
            if image_base64:
                payload_dict["ImageBase64"] = image_base64
            ...
            if "Response" not in data or "JobId" not in data["Response"]:
                raise Exception(f"Hunyuan Sketch error: {data}")
                ...
                if "Response" in data:
                    ...
                    if status == "DONE":
                        ...
                        if result_files and "Url" in result_files[0]:
                            ...
                            glb_resp = safe_request("get", glb_url, timeout=120)
                            if glb_resp.status_code != 200:
                                raise Exception("Не удалось скачать GLB (Sketch)")
                            print(f"Hunyuan Sketch: возвращаю GLB, размер: {len(glb_resp.content)} байт")
                            return glb_resp.content, "glb"
                    elif status == "FAIL":
            ...
            print(f"Hunyuan Sketch ошибка в попытке {attempt+1}: {e}")
            if attempt == max_retries - 1:
                raise e
            time.sleep(15)

    raise Exception("Hunyuan Sketch: не удалось сгенерировать модель")


# ========== HUNYUAN 3D PART API (AI3D endpoint) ==========
def submit_hunyuan_part_job(fbx_url):
    """SubmitHunyuan3DPartJob — разбивка FBX-модели на части.
    Использует AI3D endpoint (ai3d.tencentcloudapi.com), а не старый Hunyuan."""
    action = "SubmitHunyuan3DPartJob"
    payload = json.dumps({"File": {"Type": "FBX", "Url": fbx_url}})
    headers = get_ai3d_headers(action, payload)
    print(f"[Part] SubmitHunyuan3DPartJob → {AI3D_HOST}")
    resp = safe_request("post", f"https://{AI3D_HOST}", headers=headers, data=payload, timeout=90)
    data = resp.json()
    print(f"[Part] Submit response: {json.dumps(data, ensure_ascii=False)[:300]}")
    if "Response" not in data or "JobId" not in data["Response"]:
...
def query_hunyuan_part_job(job_id):
    """QueryHunyuan3DPartJob — проверка статуса разбивки.
    Использует AI3D endpoint."""
...
# ========== FALLBACK: РЕГЕНЕРАЦИЯ С ResultFormat='FBX' ==========
def hunyuan_generate_for_part(prompt=None, image_base64=None, max_retries=2):
    """Если Convert3DFormat GLB→FBX не сработал — регенерируем модель
    сразу в формате FBX через SubmitHunyuanTo3DProJob с ResultFormat='FBX'.
    Использует AI3D API (ai3d.tencentcloudapi.com).
    Возвращает FBX URL (строку) или None."""
    for attempt in range(max_retries):
        try:
            print(f"[Fallback] Регенерация FBX, попытка {attempt+1}/{max_retries}")
            action = "SubmitHunyuanTo3DProJob"
            payload_dict = {
                "Model": "3.1",
                "EnablePBR": True,
                "GenerateType": "Normal",
                "FaceCount": 1000000,
                "ResultFormat": "FBX"  # ★ ключевой параметр ★
            }
            if prompt:
            ...
            if not prompt and not image_base64:
                print("[Fallback] Нет ни prompt, ни image — пропускаю")
                return None

            payload = json.dumps(payload_dict)
            headers = get_ai3d_headers(action, payload)  # ★ AI3D headers ★
            resp = safe_request("post", f"https://{AI3D_HOST}", headers=headers, data=payload, timeout=90)
            data = resp.json()
            print(f"[Fallback] Submit response: {json.dumps(data, ensure_ascii=False)[:300]}")

            if "Response" not in data or "JobId" not in data["Response"]:
                err = data.get("Response", {}).get("Error", {}).get("Message", str(data))
                raise Exception(f"Fallback submit error: {err}")

            job_id = data["Response"]["JobId"]
            print(f"[Fallback] JobId: {job_id}")

            # Поллинг до DONE
            for _ in range(40):
                time.sleep(5)
                poll_payload = json.dumps({"JobId": job_id})
                poll_headers = get_ai3d_headers("QueryHunyuanTo3DProJob", poll_payload)
                poll_resp = safe_request("post", f"https://{AI3D_HOST}", headers=poll_headers, data=poll_payload, timeout=90)
                poll_data = poll_resp.json()

                if "Response" in poll_data:
                    status = poll_data["Response"].get("Status")
                    if status == "DONE":
                        result_files = poll_data["Response"].get("ResultFile3Ds", [])
                        for f in result_files:
                            if f.get("Type", "").upper() == "FBX" and f.get("Url"):
                                print(f"[Fallback] FBX URL получен: {f['Url'][:80]}...")
                                return f["Url"]
                        # Если FBX не в ResultFile3Ds, берём первый URL
                        if result_files and result_files[0].get("Url"):
                            print(f"[Fallback] Беру первый URL (тип: {result_files[0].get('Type', '?')})")
                            return result_files[0]["Url"]
                        raise Exception("[Fallback] Нет ResultFile3Ds после DONE")
                    elif status == "FAIL":
                        err = poll_data["Response"].get("ErrorMessage", "неизвестная ошибка")
                        raise Exception(f"[Fallback] FAILED: {err}")

            raise Exception("[Fallback] Тайм-аут ожидания (200 сек)")

        except Exception as e:
            print(f"[Fallback] Ошибка в попытке {attempt+1}: {e}")
            if attempt == max_retries - 1:
                print(f"[Fallback] Все попытки исчерпаны: {e}")
                return None
            time.sleep(15)
...
# ========== ГЕНЕРАЦИЯ ПО ЧЕРТЕЖУ ==========
def process_drawing_generation(chat_id, user_id, image_base64, gen_type):
    """Фоновая генерация по чертежу: вызывает hunyuan_generate_from_drawing
    (Model 3.0, Sketch) и отправляет результат."""
    try:
        prompt = user_last_prompt.get(user_id, "")
        send_message(chat_id, t(user_id, "drawing_generating"))
        model_data, model_format = hunyuan_generate_from_drawing(
            prompt=prompt, image_base64=image_base64, max_retries=3, user_id=user_id
        )
        use_generation(user_id, gen_type)
        update_stats(user_id)
        ext = "stl" if model_format == "stl" else "glb"
        send_document(chat_id, model_data,
                     filename=f"model_from_drawing.{ext}",
                     caption=t(user_id, "model_ready",
                              service=user_service_choice.get(user_id, "Hunyuan"),
                              fmt=user_format_choice.get(user_id, "stl").upper(),
                              prompt=prompt or "(drawing)"))
    except Exception as e:
        send_message(chat_id, t(user_id, "error", error=str(e)))
        send_alert(f"Drawing error {user_id}: {str(e)}")
    finally:
...
def process_part_generation(chat_id, user_id, glb_url, gen_type, job_id=None, image_base64=None):
    """Фоновая разбивка:
    0. Перезапрос свежего GLB URL по JobId (URL мог истечь)
    1. Convert3DFormat GLB->FBX на AI3D (3 попытки с паузами)
    2. Разбивка через Hunyuan3D-Part на AI3D
    3. Fallback: если конвертация не удалась — регенерация с ResultFormat='FBX'
    """
    last_error = None
    try:
        # === Шаг 0: Перезапрос свежего GLB URL по JobId ===
        if job_id:
            print(f"[Part] Перезапрашиваю JobId {job_id} для свежего GLB URL...")
            try:
                poll_payload = json.dumps({"JobId": job_id})
                poll_headers = get_tencent_headers("QueryHunyuanTo3DProJob", poll_payload)
                poll_resp = safe_request("post", f"https://{HUNYUAN_HOST}",
                                        headers=poll_headers, data=poll_payload, timeout=90)
                poll_data = poll_resp.json()
                if "Response" in poll_data and poll_data["Response"].get("Status") == "DONE":
                    result_files = poll_data["Response"].get("ResultFile3Ds", [])
                    if result_files and "Url" in result_files[0]:
                        fresh_url = result_files[0]["Url"]
                        print(f"[Part] Свежий GLB URL: {fresh_url[:80]}...")
                        glb_url = fresh_url
                        user_last_glb_url[user_id] = fresh_url
            except Exception as e:
                print(f"[Part] Предупреждение: не удалось обновить URL ({e}), пробую старый")

        # === Шаг 1: Конвертируем GLB в FBX через Convert3DFormat на AI3D ===
        send_message(chat_id, t(user_id, "part_converting"))
        fbx_url = None

        for conv_attempt in range(3):
            try:
                print(f"[Part] Convert3DFormat попытка {conv_attempt+1}/3")
                conv_action = "Convert3DFormat"
                conv_payload = json.dumps({"File3D": glb_url, "Format": "FBX"})
                conv_headers = get_ai3d_headers(conv_action, conv_payload)  # ★ AI3D ★
                conv_resp = safe_request("post", f"https://{AI3D_HOST}",
                                        headers=conv_headers, data=conv_payload, timeout=120)
                conv_data = conv_resp.json()
                print(f"[Part] Convert3DFormat ответ: {json.dumps(conv_data, ensure_ascii=False)[:300]}")

                if "Response" in conv_data:
                    resp_body = conv_data["Response"]
                    if "Error" in resp_body:
                        last_error = resp_body["Error"].get("Message", str(resp_body["Error"]))
                        print(f"[Part] Convert3DFormat ошибка: {last_error}")
                        time.sleep(3 + conv_attempt * 2)
                        continue
                    # Асинхронный ответ (JobId вместо ResultFile3D)
                    if "JobId" in resp_body:
                        conv_job = resp_body["JobId"]
                        print(f"[Part] Convert3DFormat вернул JobId: {conv_job}, поллинг...")
                        for _ in range(12):
                            time.sleep(5)
                            poll_payload = json.dumps({"JobId": conv_job})
                            poll_headers = get_ai3d_headers("QueryConvert3DFormatJob", poll_payload)
                            poll_resp = safe_request("post", f"https://{AI3D_HOST}",
                                                    headers=poll_headers, data=poll_payload, timeout=60)
                            poll_data = poll_resp.json()
                            if "Response" in poll_data:
                                pr = poll_data["Response"]
                                if pr.get("Status") == "DONE":
                                    if "ResultFile3D" in pr:
                                        fbx_url = pr["ResultFile3D"]
                                        break
                                    if "ResultFile3Ds" in pr and pr["ResultFile3Ds"]:
                                        fbx_url = pr["ResultFile3Ds"][0].get("Url")
                                        break
                                elif pr.get("Status") == "FAIL":
                                    last_error = pr.get("ErrorMessage", "Convert3DFormat FAILED")
                                    break
                        if fbx_url:
                            break
                        else:
                            last_error = last_error or "Convert3DFormat polling timeout"
                            time.sleep(3)
                            continue
                    # Синхронный ответ с ResultFile3D
                    if "ResultFile3D" in resp_body:
                        fbx_url = resp_body["ResultFile3D"]
                        break
                else:
                    last_error = str(conv_data)[:200]
                    time.sleep(3 + conv_attempt * 2)
                    continue

            except Exception as e:
                last_error = str(e)
                print(f"[Part] Convert3DFormat исключение: {e}")
                time.sleep(3 + conv_attempt * 2)

        # === Fallback: регенерируем модель с ResultFormat='FBX' ===
        if not fbx_url:
            print(f"[Part] Convert3DFormat не сработал ({last_error}), запускаю fallback...")
            prompt = user_last_prompt.get(user_id, None)
            img = image_base64 or user_last_image.get(user_id, None)
            if prompt or img:
                send_message(chat_id, t(user_id, "part_fallback"))
                fbx_url = hunyuan_generate_for_part(prompt=prompt, image_base64=img)
                if fbx_url:
                    print(f"[Part] Fallback FBX URL: {fbx_url[:80]}...")

        if not fbx_url:
            raise Exception(f"Не удалось получить FBX: {last_error or 'неизвестная ошибка'}")

        # === Шаг 2: Разбиваем на части через Hunyuan3D-Part ===
        send_message(chat_id, t(user_id, "part_splitting"))
        parts = hunyuan_part_generate(fbx_url)

        # === Шаг 3: Отправляем каждую часть ===
        use_generation(user_id, gen_type)
        update_stats(user_id)
...
def get_main_keyboard(user_id):
    ...
    return {
        "inline_keyboard": [
            ...
            [{"text": "📐 По чертежу" if lang == "ru" else "📐 From Drawing", "callback_data": "gen_drawing"}],
...
def poll():
    if not TELEGRAM_BOT_TOKEN:
        print('⚠️ poll: no TELEGRAM_BOT_TOKEN', flush=True)
        return
                ...
                for update in updates:
                    ...
                    if "callback_query" in update:
                        ...
                        if data == "menu":
                            ...
                            send_message(chat_id, "🔧 Выберите способ генерации:", keyboard=choice_keyboard)
                        elif data == "gen_drawing":
                            user_states[chat_id] = "awaiting_drawing"
                            send_message(chat_id, t(user_id, "drawing_start"), keyboard=cancel_keyboard)
                        ...
                        # ★ НОВЫЕ ФИЧИ ★
                            ...
                            job_id = user_last_hunyuan_job.get(user_id)
                            image_base64 = user_last_image.get(user_id, None)
                            send_message(chat_id, t(user_id, "part_split_start"))
                            executor.submit(process_part_generation, chat_id, user_id, glb_url, gen_type, job_id, image_base64)
                    ...
                    if "message" in update:
                        ...
                        if chat_id in user_states:
                            ...
                            if state == "awaiting_text" and "text" in msg:
                            ...
                            elif state == "awaiting_drawing" and "photo" in msg:
                                del user_states[chat_id]
                                rate_ok, rate_msg = check_rate_limit(user_id)
                                if not rate_ok:
                                    send_message(chat_id, t(user_id, "rate_limit", msg=rate_msg), keyboard=get_main_keyboard(user_id))
                                ...
                                user_busy[user_id] = True
                                send_message(chat_id, t(user_id, "drawing_generating"))
                                def drawing_task(chat_id, user_id, gen_type, msg):
                                    try:
                                        file_id = msg["photo"][-1]["file_id"]
                                        file_info = requests.get(API_URL + f"/getFile?file_id={file_id}").json()
                                        file_url = f"https://api.telegram.org/file/bot{TELEGRAM_BOT_TOKEN}/{file_info['result']['file_path']}"
                                        photo_data = requests.get(file_url).content
                                        image_base64 = base64.b64encode(photo_data).decode('utf-8')
                                        process_drawing_generation(chat_id, user_id, image_base64, gen_type)
                                    except Exception as e:
                                        send_message(chat_id, f"❌ Ошибка загрузки чертежа: {str(e)}")
                                        user_busy[user_id] = False
                                executor.submit(drawing_task, chat_id, user_id, gen_type, msg)
                                continue
                                ...
                                elif state == "awaiting_drawing":
                                    user_last_prompt[user_id] = msg["text"]
                                    send_message(chat_id, "📝 Описание сохранено! Теперь отправьте чертёж или эскиз.", keyboard=cancel_keyboard)
                                continue
...
def start_bot_background():
    """Long-polling in background thread (only with TELEGRAM_BOT_TOKEN)."""
    if not TELEGRAM_BOT_TOKEN:
        print("⚠️ [DEBUG] TELEGRAM_BOT_TOKEN empty — polling OFF", flush=True)
        return
    print("🤖 Bot polling started", flush=True)
    threading.Thread(target=poll, daemon=True, name="tg-poll").start()


# Import-time start (Render / gunicorn / python app.py)
