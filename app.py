"""
@Kostya_3d_bot — Telegram-бот для генерации 3D-моделей.
Поддерживает Hunyuan3D (Tencent), Meshy AI.
Версия: 2.1-final | 2026-06-28
"""


import os, json, time, threading, requests, hashlib, hmac, base64
...
# ========== ПУЛ ПОТОКОВ И ГЛОБАЛЬНОЕ СОСТОЯНИЕ ==========
...
# ========== БЛОКИРОВКА ДЛЯ АТОМАРНОЙ УСТАНОВКИ user_busy ==========
_busy_lock = threading.Lock()


def try_acquire_busy(user_id):
    with _busy_lock:
...
TEXTS = {
    ...
    "en": {
        ...
        "refine_prompt": "📝 Your last prompt: <b>{prompt}</b>\n\nDescribe what to change:",
...
def check_rate_limit(user_id):
    ...
    recent = [t for t in user_rate_limit[user_id] if t > now - 60]
    if len(recent) >= USER_REQUESTS_PER_MINUTE:
...
def escape_html(text):
    """Экранирует <, >, & для безопасного HTML в Telegram."""
...
def _tg_post(method, **kwargs):
    try:
        resp = requests.post(API_URL + method, timeout=kwargs.pop("_timeout", 15), **kwargs)
        if resp.status_code == 200:
            return resp.json()
        print(f"⚠️ Telegram API error: {method} -> {resp.status_code} {resp.text[:200]}", flush=True)
        return None
    except Exception as e:
        print(f"⚠️ Telegram API exception ({method}): {e}", flush=True)
        return None


def send_alert(message):
    if ADMIN_CHAT_ID:
        _tg_post("/sendMessage", json={"chat_id": ADMIN_CHAT_ID, "text": f"⚠️ АЛЕРТ БОТА:\n{message}", "parse_mode": "HTML"}, _timeout=5)


def send_message(chat_id, text, keyboard=None):
    """Отправка сообщения с АВТОМАТИЧЕСКИМ экранированием HTML."""
    text = escape_html(str(text))
    payload = {"chat_id": chat_id, "text": text, "parse_mode": "HTML"}
    if keyboard:
        payload["reply_markup"] = json.dumps(keyboard)
    _tg_post("/sendMessage", json=payload, _timeout=10)
...
def send_document(chat_id, file_content, filename="model.stl", caption=""):
    ...
    try:
        ...
        resp = requests.post(API_URL + "/sendDocument", files=files, data=data, timeout=60)
        if resp.status_code != 200:
            print(f"⚠️ sendDocument error: {resp.status_code} {resp.text[:200]}", flush=True)
            send_message(chat_id, "❌ Не удалось отправить файл. Попробуйте ещё раз.")
            return False
        return True
    except Exception as e:
        print(f"⚠️ sendDocument exception: {e}", flush=True)
        send_message(chat_id, "❌ Ошибка отправки файла.")
        return False


def send_invoice(chat_id, title, description, payload, stars_amount):
    _tg_post("/sendInvoice", json={
...
def load_stats():
    ...
    if _stats_cache is None:
        _stats_cache = {"total_generations": 0, "total_users": [], "daily_stats": {}, "user_generations": {}}
    return _stats_cache
...
def update_stats(user_id):
    ...
    if user_id not in stats.get("total_users", []):
        stats.setdefault("total_users", []).append(user_id)
    today = datetime.now().strftime("%Y-%m-%d")
    if today not in stats.get("daily_stats", {}):
        stats["daily_stats"][today] = 0
    stats["daily_stats"][today] += 1
    stats.setdefault("user_generations", {})[str(user_id)] = stats["user_generations"].get(str(user_id), 0) + 1
    save_stats(stats)
...
def send_stats(chat_id):
    ...
    text = (
        ...
        f"👥 Всего пользователей: {len(stats.get('total_users', []))}\n"
        f"🎲 Всего генераций: {stats.get('total_generations', 0)}\n"
        f"📅 Генераций сегодня: {stats.get('daily_stats', {}).get(today, 0)}\n"
    )
    user_gens = stats.get("user_generations", {})
    if user_gens:
        top_users = sorted(user_gens.items(), key=lambda x: int(x[1]) if isinstance(x[1], str) else x[1], reverse=True)[:10]
        text += "\n<b>🏆 Топ пользователей по генерациям:</b>\n"
...
def hunyuan_generate_with_retry(prompt, output_format="stl", max_retries=3):
    for attempt in range(max_retries):
        try:
            print(f"Hunyuan: попытка {attempt+1}, формат={output_format}")
                    ...
                    if status == "DONE":
                        ...
                        if result_files and "Url" in result_files[0]:
                            ...
                            if output_format == "glb":
                                print(f"Hunyuan: GLB готов, {len(glb_data)} байт")
                                return glb_data, "glb"
                            ...
                            if "Response" in conv_data and "ResultFile3D" in conv_data["Response"]:
                                ...
                                if stl_resp.status_code == 200:
                                    print(f"Hunyuan: STL готов, {len(stl_resp.content)} байт")
            ...
            if attempt < max_retries - 1:
                time.sleep(15)
                continue
            raise Exception("Hunyuan: тайм-аут ожидания")
        except Exception as e:
            print(f"Hunyuan ошибка (попытка {attempt+1}): {e}")
            if attempt == max_retries - 1:
                raise e
            time.sleep(15)
...
def hunyuan_generate_from_photo(image_base64, output_format="stl", max_retries=3):
    for attempt in range(max_retries):
        try:
            print(f"Hunyuan фото: попытка {attempt+1}, формат={output_format}")
            action = "SubmitHunyuanTo3DProJob"
            payload_dict = {
                "Model": "3.1",
            ...
            data = resp.json()
            if "Response" not in data or "JobId" not in data["Response"]:
                raise Exception(f"Hunyuan фото error: {data}")
            job_id = data["Response"]["JobId"]
            query_action = "QueryHunyuanTo3DProJob"
            for _ in range(40):
                time.sleep(5)
                ...
                if "Response" in data:
                    ...
                    if status == "DONE":
                        ...
                        if result_files and "Url" in result_files[0]:
                            ...
                            glb_data = glb_resp.content
                            if output_format == "glb":
                                print(f"Hunyuan фото: GLB готов, {len(glb_data)} байт")
                                return glb_data, "glb"
                            conv_action = "Convert3DFormat"
                            conv_payload = json.dumps({"File3D": glb_url, "Format": "STL"})
                            conv_headers = get_tencent_headers(conv_action, conv_payload)
                            ...
                            if "Response" in conv_data and "ResultFile3D" in conv_data["Response"]:
                                ...
                                stl_resp = safe_request("get", stl_url, timeout=120)
                                if stl_resp.status_code == 200:
                                    print(f"Hunyuan фото: STL готов, {len(stl_resp.content)} байт")
                                    return stl_resp.content, "stl"
                            raise Exception("Не удалось конвертировать в STL")
                    elif status == "FAIL":
                        raise Exception("Generation FAILED")
            ...
            print(f"Hunyuan фото ошибка (попытка {attempt+1}): {e}")
            if attempt == max_retries - 1:
                raise e
            time.sleep(15)
...
def meshy_generate_with_retry(prompt, output_format="stl", max_retries=2):
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(f"Meshy ошибка (попытка {attempt+1}): {e}")
            if attempt == max_retries - 1:
                raise e
            time.sleep(15)
    ...
    try:
        ...
        if service == "meshy" and MESHY_API_KEY:
            try:
                ...
                print(f"Meshy упал, fallback на Hunyuan: {e}")
                try:
                    model, fmt = hunyuan_generate_with_retry(prompt, output_format)
        ...
        if model:
            ...
            caption = t(user_id, "model_ready", service=used_service.upper(), fmt=fmt.upper(), prompt=escape_html(prompt[:100]))
            if gen_type == "free" and user_id != ADMIN_CHAT_ID:
                caption += t(user_id, "free_remaining", remaining=remaining, limit=FREE_LIMIT)
            if send_document(chat_id, model, filename=filename, caption=caption):
                update_stats(user_id)
            user_last_prompt[user_id] = prompt
        else:
            send_message(chat_id, t(user_id, "error_generation"))
    except Exception as e:
        send_message(chat_id, t(user_id, "error", error=escape_html(str(e))))
...
def process_photo_generation(chat_id, user_id, image_base64, gen_type):
    output_format = user_format_choice.get(user_id, "stl")
    try:
        model, fmt = hunyuan_generate_from_photo(image_base64, output_format)
        if model:
            use_generation(user_id, gen_type)
...
def forward_to_admin(user_id, username, first_name, text):
    ...
    try:
        ...
        admin_msg = (
            ...
            f"👤 {safe_name} ({escape_html(user_tag)})\n"
            f"━━━━━━━━━━━━━━━━\n"
            f"💬 {safe_text}\n"
            f"━━━━━━━━━━━━━━━━\n\n"
            f"💡 <i>Нажмите «Ответить» и напишите сообщение</i>"
        )
        _tg_post("/sendMessage", json={
            "chat_id": ADMIN_CHAT_ID, "text": admin_msg,
            "parse_mode": "HTML", "reply_markup": json.dumps(reply_keyboard)
        }, _timeout=10)
    except Exception as e:
...
def reply_to_user(admin_message_text):
    if ADMIN_CHAT_ID not in pending_reply:
        send_message(ADMIN_CHAT_ID, "ℹ️ Нет активного диалога.")
        return
    user_id = pending_reply[ADMIN_CHAT_ID]
    safe_text = escape_html(admin_message_text)
    send_message(user_id, f"📨 <b>Ответ разработчика:</b>\n\n{safe_text}")
    send_keyboard = {
        "inline_keyboard": [
    ...
    send_message(ADMIN_CHAT_ID, f"✅ Ответ отправлен пользователю <code>{user_id}</code>", keyboard=send_keyboard)
...
def poll():
    ...
    print("🟢 [POLL] Запуск long-polling...", flush=True)
    ...
    while True:
        try:
            ...
            if resp.status_code == 200:
                ...
                if updates:
                    print(f"!!! [DEBUG] Получено {len(updates)} обновлений", flush=True)
                for update in updates:
                    last_update_id = update["update_id"]
                    
                    if "pre_checkout_query" in update:
                        pq = update["pre_checkout_query"]
                        _tg_post("/answerPreCheckoutQuery", json={"pre_checkout_query_id": pq["id"], "ok": True}, _timeout=5)
                        continue
                    ...
                    if "callback_query" in update:
                        ...
                        user_id = cb["from"]["id"]
                        data = cb["data"]
                        _tg_post("/answerCallbackQuery", json={"callback_query_id": cb["id"]}, _timeout=5)
                        ...
                        if data == "menu":
                            ...
                            if user_id in user_last_prompt:
                                ...
                                safe_prompt = escape_html(user_last_prompt[user_id][:100])
                                send_message(chat_id, t(user_id, "refine_prompt", prompt=safe_prompt), keyboard=cancel_keyboard)
                            else:
                            ...
                            send_message(chat_id, f"✏️ Теперь напишите сообщение для пользователя <code>{target_user_id}</code>.\n/cancelreply для отмены.")
                    ...
                    if "message" in update:
                        ...
                        if "text" in msg:
                            print(f"!!! [DEBUG] user_id={user_id}, chat_id={chat_id}, текст={msg['text'][:50]}", flush=True)
                        ...
                        if chat_id in user_states:
                            ...
                            if state == "awaiting_refine" and "text" in msg:
                                ...
                                executor.submit(_generation_wrapper, process_text_generation, chat_id, user_id, new_prompt, gen_type)
                                continue
                            ...
                            if state == "awaiting_text" and "text" in msg:
                                ...
                                fmt = user_format_choice.get(user_id, "stl")
                                send_message(chat_id, t(user_id, "generating", service=service.upper(), fmt=fmt.upper()))
                                executor.submit(_generation_wrapper, process_text_generation, chat_id, user_id, prompt, gen_type)
                                continue
                                ...
                                executor.submit(_photo_task, chat_id, user_id, gen_type, msg)
                                continue
...
def _generation_wrapper(func, chat_id, user_id, *args, **kwargs):
    try:
        func(chat_id, user_id, *args, **kwargs)
    finally:
        release_busy(user_id)


def _photo_task(chat_id, user_id, gen_type, msg):
    try:
        file_id = msg["photo"][-1]["file_id"]
        file_info_resp = requests.get(API_URL + f"/getFile?file_id={file_id}").json()
        ...
        send_message(chat_id, f"❌ Ошибка загрузки фото: {escape_html(str(e))}")
    finally:
        release_busy(user_id)
