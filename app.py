...
user_blueprint_images = {}  # {user_id: [file_id, file_id, file_id]}
user_blueprint_step = {}    # {user_id: 0/1/2} — какой ракурс загружаем
...
TEXTS = {
    "ru": {
        ...
        "blueprint_front": "📐 Загрузите фото *СПЕРЕДИ* (вид спереди).",
        "blueprint_side": "📐 Загрузите фото *СБОКУ* (вид слева).",
        "blueprint_top": "📐 Загрузите фото *СВЕРХУ*.",
        "blueprint_ready": "📐 Все 3 фото загружены. Начинаю генерацию...",
    ...
    "en": {
        ...
        "blueprint_front": "📐 Upload the *FRONT* view photo.",
        "blueprint_side": "📐 Upload the *SIDE* view photo (left side).",
        "blueprint_top": "📐 Upload the *TOP* view photo.",
        "blueprint_ready": "📐 All 3 photos uploaded. Starting generation...",
                ...
                if "Response" in data:
                    ...
                    if status == "DONE":
                        ...
                        if result_files and "Url" in result_files[0]:
                            ...
                            # Если нужен GLB — скачиваем сразу
                            if output_format.lower() == "glb":
                                model_resp = safe_request("get", glb_url, timeout=120)
                                if model_resp.status_code == 200:
                                    return model_resp.content, "glb"
                            
                            # Если нужен STL — конвертируем
...
# ===== ИСПРАВЛЕННАЯ ФУНКЦИЯ ГЕНЕРАЦИИ ПО MULTIVIEW =====
def hunyuan_generate_from_multiview(image_urls_list, output_format="stl", max_retries=3):
    """Генерация по MultiView с выбором формата (STL/GLB)"""
    VIEW_ORDER = ["left", "right", "top", "bottom", "back", "left_front", "right_front"]
    
    multiview = []
    for i, url in enumerate(image_urls_list):
        view_type = VIEW_ORDER[i] if i < len(VIEW_ORDER) else f"view_{i}"
        multiview.append({"ViewType": view_type, "ImageUrl": url})
    
    for attempt in range(max_retries):
        try:
            print(f"Hunyuan MultiView: попытка {attempt+1}, формат: {output_format}")
            action = "SubmitHunyuanTo3DProJob"
            payload_dict = {
                "Model": "3.1",
                "MultiViewImages": multiview,
                "EnablePBR": True,
                "GenerateType": "Normal",
                "FaceCount": 1000000
            }
...
def process_blueprint_generation(chat_id, user_id, gen_type):
    ...
    if len(image_urls) < 3:
        send_message(chat_id, "⚠️ Нужно 3 фото. Начните заново.")
        user_blueprint_images.pop(user_id, None)
        user_busy[user_id] = False
        return
    
    try:
        model, fmt = hunyuan_generate_from_multiview(image_urls, output_format)
        if model:
            use_generation(user_id, gen_type)
            update_stats(user_id)
            remaining = max(0, FREE_LIMIT - user_free_used.get(user_id, 0))
            filename = f"model.{fmt}"
            caption = t(user_id, "model_ready", service="HUNYUAN", fmt=fmt.upper(), prompt="MultiView")
            if gen_type == "free" and user_id != ADMIN_CHAT_ID:
                caption += t(user_id, "free_remaining", remaining=remaining, limit=FREE_LIMIT)
            send_document(chat_id, model, filename=filename, caption=caption)
        else:
        ...
        send_message(chat_id, t(user_id, "error", error=str(e)))
        send_alert(f"Ошибка генерации MultiView у {user_id}: {str(e)}")
    finally:
        user_blueprint_images.pop(user_id, None)
        user_busy[user_id] = False
                ...
                for update in updates:
                    ...
                    if "callback_query" in update:
                        ...
                        if data == "menu":
                            ...
                            send_message(chat_id, t(user_id, "blueprint_front"), keyboard=cancel_keyboard)
                        elif data == "buy_one":
                    ...
                    if "message" in update:
                        ...
                        if chat_id in user_states:
                            ...
                            # Приём фото для MultiView (3 фото по порядку)
                            if state == "awaiting_blueprint" and "photo" in msg:
                                ...
                                if count == 1:
                                    send_message(chat_id, t(user_id, "blueprint_side"), keyboard=cancel_keyboard)
                                elif count == 2:
                                    send_message(chat_id, t(user_id, "blueprint_top"), keyboard=cancel_keyboard)
                                elif count >= 3:
                                    del user_states[chat_id]
                                    ...
                                    send_message(chat_id, t(user_id, "blueprint_ready"))
