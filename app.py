...
TEXTS = {
    "ru": {
        ...
        "blueprint_start": "📐 *Генерация по чертежу*\n\nЗагрузите ОДНО фото чертежа (на нём могут быть виды с разных сторон).",
        "blueprint_prompt": "📝 Напишите короткое описание объекта (материал, размер).",
        "blueprint_ready": "📐 Начинаю генерацию по чертежу...",
    ...
    "en": {
        ...
        "blueprint_start": "📐 *Blueprint Generation*\n\nUpload ONE photo of the blueprint (it can have views from different sides).",
        "blueprint_prompt": "📝 Write a short description of the object (material, size).",
        "blueprint_ready": "📐 Starting blueprint generation...",
...
def hunyuan_generate_from_blueprint(image_base64, prompt, output_format="stl", max_retries=3):
    """Генерация по чертежу (одно фото)"""
    for attempt in range(max_retries):
        try:
            print(f"Hunyuan Blueprint: попытка {attempt+1}")
            action = "SubmitHunyuanTo3DProJob"
            payload_dict = {
                "Model": "3.1",
            ...
            print(f"Hunyuan Blueprint ошибка в попытке {attempt+1}: {e}")
            if attempt == max_retries - 1:
                raise e
            time.sleep(15)
    raise Exception("Hunyuan Blueprint: не удалось сгенерировать модель")
...
def process_blueprint_generation(chat_id, user_id, image_base64, gen_type):
    """Генерация по чертежу (одно фото)"""
    output_format = user_format_choice.get(user_id, "stl")
    prompt = user_last_prompt.get(user_id, "3D object from blueprint")
    try:
        model, fmt = hunyuan_generate_from_blueprint(image_base64, prompt, output_format)
        if model:
            use_generation(user_id, gen_type)
            update_stats(user_id)
            remaining = max(0, FREE_LIMIT - user_free_used.get(user_id, 0))
                    ...
                    if "message" in update:
                        ...
                        if chat_id in user_states:
                            ...
                            if state == "awaiting_blueprint" and "photo" in msg:
                                ...
                                def blueprint_task(chat_id, user_id, gen_type, file_id):
                                    try:
                                        ...
                                        process_blueprint_generation(chat_id, user_id, image_base64, gen_type)
                                    except Exception as e:
                                        send_message(chat_id, t(user_id, "error", error=str(e)))
                                        user_busy[user_id] = False
                                
                                executor.submit(blueprint_task, chat_id, user_id, gen_type, file_id)
                                continue
