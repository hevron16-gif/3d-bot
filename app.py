...
def generate_3d_model(prompt=None, image_base64=None, image_caption=None):
    ...
    if image_base64:
        print(f"[IMAGE-TO-3D] Получено фото, размер base64: {len(image_base64)} символов")
        ...
        print(f"[TEXT-TO-3D] Промпт: {prompt[:100]}...")
    ...
    print(f"[DEBUG] Создание задачи: статус {response.status_code}")
    ...
    print(f"[DEBUG] Задача создана: {task_id}")
    ...
    raise Exception("Таймаут генерации (>7 минут)")
            ...
            if resp.status_code == 200:
                ...
                for update in updates:
                    ...
                    if "callback_query" in update:
                        ...
                        if data == "buy_premium":
                            # Здесь будет функция оплаты
                            send_message(chat_id, "💎 Premium пока в разработке (199 ⭐). Скоро будет доступен.")
                        elif data == "gen_text":
                    ...
                    # === ОБЫЧНЫЕ СООБЩЕНИЯ ===
                        ...
                        if msg.get("text") and user_states.get(chat_id) == "awaiting_text":
                            ...
                            if tier != "premium" and left <= 0:
                                ...
                                continue
                            send_message(chat_id, "⏳ Генерирую модель по тексту...")
                            try:
                        ...
                        # Генерация по фото
                            ...
                            if tier != "premium" and left <= 0:
                                send_message(chat_id, "❌ Лимит исчерпан.")
                                continue
                            send_message(chat_id, "⏳ Обрабатываю фото... (может занять 2–6 минут)")
                            try:
                                ...
                                send_message(chat_id, f"❌ Ошибка обработки фото: {str(e)[:350]}")
