...
def send_stats(chat_id):
    stats = load_stats()
    today = datetime.now().strftime("%Y-%m-%d")
    premium_count = sum(1 for t in user_subscription.values() if t > time.time())
    text = (
        f"📊 *Статистика бота*\n\n"
        ...
        f"💎 *Премиум активна у:* {premium_count} пользователей\n"
        f"🎟 *Разовых генераций куплено:* {len(user_paid_one)}"
    )
                ...
                for update in resp.json().get("result", []):
                    ...
                    if "callback_query" in update:
                        ...
                        if data == "menu":
                            ...
                            send_message(chat_id, "📝 Отправьте текстовое описание модели.", keyboard=cancel_keyboard)
                        elif data == "gen_photo":
                            user_states[chat_id] = "awaiting_photo"
                            send_message(chat_id, "📸 Отправьте фото объекта.", keyboard=cancel_keyboard)
                        elif data == "cancel":
                    ...
                    if "message" in update:
                        ...
                        if chat_id in user_states and user_states[chat_id] == "awaiting_text" and "text" in msg:
                            ...
                            if not can:
                                send_message(chat_id, "❌ Бесплатные генерации закончились.", keyboard=main_keyboard)
                                continue
                            send_message(chat_id, "⏳ Генерирую модель по фото...")
                            try:
                        ...
                        elif "text" in msg and msg["text"] and not msg["text"].startswith('/') and user_id != ADMIN_CHAT_ID:
