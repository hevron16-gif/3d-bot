...
# ЦЕНЫ (Stars)
PRICE_SINGLE = 50          # Разовая генерация
PRICE_PACK_5 = 200         # Пакет 5 шт.
PRICE_SUB_LITE = 150       # Подписка Lite (20 ген/мес)
PRICE_SUB_PRO = 300        # Подписка Pro (безлимит)
PRICE_PART_API = 80        # Part API (разделение FBX)
...
def db_get_user(user_id):
    ...
    conn.commit()
    conn.close()
    keys = ['user_id','free_used','paid_one','subscription_until','sub_type','sub_generations_used','service','format']
...
def check_rate_limit(user_id):
    ...
    if per_min >= REQ_PER_MINUTE:
        conn.close(); return False, "Слишком много запросов. Подождите минуту."
    if per_hour >= REQ_PER_HOUR:
        conn.close(); return False, "Лимит запросов на час исчерпан."
...
def can_generate(user_id):
    ...
    # Проверка подписки Lite (20 ген/мес)
...
def main_keyboard(user_id):
    ...
    return {"inline_keyboard": [
        ...
        [{"text": f"💎 Lite подписка ({PRICE_SUB_LITE}⭐/мес)", "callback_data": "sub_lite"}],
        [{"text": f"💎 Pro подписка ({PRICE_SUB_PRO}⭐/мес)", "callback_data": "sub_pro"}],
        [{"text": "🧩 Part API (разделить FBX)", "callback_data": "part_menu"}],
            ...
            for update in updates:
                ...
                if "message" in update and "successful_payment" in update["message"]:
                    ...
                    if pl == "single_generation":
                        ...
                        send_message(cid, f"✅ Разовая генерация активирована!")
                    elif pl == "pack_5":
                        db_update_user(uid, paid_one=1)  # Используем тот же флаг или добавьте отдельную логику
                        send_message(cid, "✅ Пакет 5 генераций активирован! (используйте по одной)")
                    elif pl == "sub_lite":
                        ...
                        # Логика Part API оплаты — можно добавить отдельный флаг
                        send_message(cid, "✅ Part API оплачен! Отправьте FBX файл.")
                    continue
                ...
                if "callback_query" in update:
                    ...
                    if data == "menu":
                        ...
                        send_message(cid, "🧩 <b>Part API</b>\n\nОтправьте FBX-файл (до 200 МБ, до 1.5M полигонов), и я разделю его на части.\n\nДля начала оплатите услугу:", {"inline_keyboard": [
                ...
                if "document" in msg and cid in user_states and user_states[cid] == "awaiting_part_fbx":
                    del user_states[cid]
                    doc = msg["document"]
                    if not doc["file_name"].lower().endswith(".fbx"):
                        send_message(cid, "❌ Только FBX-файлы.", main_keyboard(uid))
                        continue
                    if doc.get("file_size", 0) > 200 * 1024 * 1024:
                        send_message(cid, "❌ Файл >200 МБ.", main_keyboard(uid))
                        continue
                    send_message(cid, "⏳ Загрузка и сегментация...")
                    # Здесь нужно: скачать файл, залить на хостинг с прямым URL, вызвать part_segment
                    # Для простоты — инструкция пользователю
                    send_message(cid, "⚠️ Для Part API модель должна быть доступна по прямой ссылке.\nЗагрузите FBX на облако и отправьте ссылку командой:\n<code>/part https://site.com/model.fbx</code>", main_keyboard(uid))
                    continue
        except Exception as e:
