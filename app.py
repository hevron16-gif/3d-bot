...
def main_keyboard():
    return {"inline_keyboard": [
        ...
        [{"text": "💎 Купить Premium (199 ⭐)", "callback_data": "buy_premium"}]
    ]}


# ========== ГЕНЕРАЦИЯ 3D (исправленные эндпоинты) ==========
def generate_3d_model(prompt=None, image_base64=None, image_caption=None):
    ...
    if image_base64:
        ...
        # Жёстко прописываем актуальный путь
    ...
    print(f"[DEBUG] Ответ Meshy: статус {response.status_code}")
    ...
    if not task_id:
        raise Exception("Не удалось получить task_id от Meshy")


    print(f"[DEBUG] Задача создана, ID: {task_id}")
    ...
    while time.time() - start_time < 420:  # максимум 7 минут
        time.sleep(10)
        ...
        if status_resp.status_code == 200:
            ...
            print(f"[DEBUG] Статус задачи: {status}")
            ...
            if status == "SUCCEEDED":
                ...
                raise Exception("Генерация не удалась (FAILED)")
        else:
...
def poll_updates():
    last_update_id = 0
    print("🟢 Начинаю прослушивание обновлений...")
                ...
                for update in updates:
                    ...
                    if "callback_query" in update:
                        ...
                        if data == "gen_text":
                            ...
                            send_message(chat_id, "📸 Отправь фото (можно с подписью).")
                        elif data == "buy_premium":
                            send_message(chat_id, "💎 Premium (199 ⭐) пока в разработке.")
                        ...
                        if msg.get("text") == "/start":
                            send_message(chat_id, "👋 Привет! Я генерирую 3D-модели для печати.\nБесплатно: 3 модели в день.", main_keyboard())
                            continue
                        ...
                        if msg.get("text") and user_states.get(chat_id) == "awaiting_text":
                            ...
                            if tier != "premium" and left <= 0:
                                send_message(chat_id, "❌ Лимит на сегодня исчерпан.")
                                continue
                            ...
                            try:
                                ...
                                send_document(chat_id, model_bytes, "model.glb", "✅ Готово!")
                            except Exception as e:
                        ...
                        # Генерация по фото
                            ...
                            if tier != "premium" and left <= 0:
                                ...
                                continue
                            send_message(chat_id, "⏳ Обрабатываю фото... (2–6 минут)")
                            try:
                                ...
                                photo_resp = requests.get(file_url, timeout=60)
                                ...
                                if tier != "premium":
                                    decrement_limit(user_id)
                                send_document(chat_id, model_bytes, "model.glb", "✅ Готово! Модель по фото")
                            except Exception as e:
