...
def convert_to_fbx(file_url):
    ...
    headers = get_tencent_headers(action, payload)  # ← СТАРЫЙ ХОСТ
    resp = safe_request("post", f"https://{HUNYUAN_HOST}", headers=headers, data=payload, timeout=120)
    data = resp.json()
                ...
                if "Response" in data:
                    ...
                    if status == "DONE":
                        ...
                        if result_files and "Url" in result_files[0]:
                            ...
                            # Конвертация в STL через Convert3DFormat (старый хост)
                            ...
                            if "Response" in conv_data and "ResultFile3D" in conv_data["Response"]:
                                ...
                                stl_resp = safe_request("get", stl_url, timeout=300)
...
def process_part_job(chat_id, user_id, file_url, gen_type):
    try:
        # Шаг 1: Конвертируем в FBX через старый хост
        send_message(chat_id, "🔄 Конвертирую в FBX...")
        fbx_url = convert_to_fbx(file_url)


        # Шаг 2: Отправляем на разделение через старый хост
