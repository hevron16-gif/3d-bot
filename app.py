...
MESHY_API_KEY = os.environ.get("MESHY_API_KEY")          # оставлен на случай, но не используется
...
def can_generate(user_id, engine):
    ...
    # для Hunyuan – 10 генераций бесплатно в день
    hunyuan_cnt = user_limits[user_id][2]
    return hunyuan_cnt < 10
...
def handle_help_info(chat_id):
    text = (
        "❓ *Что умеет бот:*\n\n"
        "🔧 *Hunyuan (Точная)* — генерирует 3D-модель по тексту или фото. Высокая точность, подходит для печати функциональных узлов.\n\n"
                ...
                for update in resp.json().get("result", []):
                    ...
                    if "callback_query" in update:
                        ...
                        if chat_id in user_states:
                            ...
                            if "text" in msg:
                                prompt = msg["text"]
                                send_message(chat_id, "⏳ Генерирую через HUNYUAN по тексту... (1-3 мин)")
                                try:
                                    model = hunyuan_generate_from_text(prompt)
                                    if model:
                                        use_generation(user_id, engine)
                                        send_document(chat_id, model, caption=f"HUNYUAN (текст): {prompt[:100]}")
                                ...
                                send_message(chat_id, "⏳ Генерирую через HUNYUAN по фото... (1-3 мин)")
                                try:
                                    ...
                                    if model:
                                        use_generation(user_id, engine)
                                        send_document(chat_id, model, caption="HUNYUAN (по фото)")
                                        send_message(chat_id, "✅ Готово!")
                                    else:
                                        send_message(chat_id, "❌ Ошибка: модель не получена.")
                                except Exception as e:
