...
def create_generation_task_text(prompt):
    ...
    payload = {
        "mode": "preview",
        "prompt": prompt,
        "art_style": "sculpture",      # лучше для фигурок и животных
        "should_remesh": True
    }
    try:
        response = requests.post(f"{MESHY_API_URL}/text-to-3d", headers=headers, json=payload, timeout=30)
        print(f"Text-to-3D status: {response.status_code}")
        if response.status_code in (200, 202):
            data = response.json()
            task_id = data.get("result") or data.get("task_id")
            if task_id:
                return {"task_id": task_id, "type": "text"}
        print(f"Text-to-3D error: {response.text[:300]}")
        return None
    except Exception as e:
...
def create_generation_task_image(image_url):
    ...
    payload = {
        "image_url": image_url,
        "should_remesh": True,
        "should_texture": True,      # включаем текстуры
        "enable_pbr": True           # лучше качество материалов
    }
    try:
        response = requests.post(f"{MESHY_API_URL}/image-to-3d", headers=headers, json=payload, timeout=30)
        print(f"Image-to-3D status: {response.status_code}")
        if response.status_code in (200, 202):
            data = response.json()
            task_id = data.get("result") or data.get("task_id")
            if task_id:
                return {"task_id": task_id, "type": "image"}
        print(f"Image-to-3D error: {response.text[:300]}")
        return None
    except Exception as e:
        print(f"Ошибка create_task_image: {e}")
        return None

# ========== ОБЩАЯ ФУНКЦИЯ ОЖИДАНИЯ (ИСПРАВЛЕНА!) ==========
def wait_for_generation(chat_id, task_id, user_id, task_type="text"):
    max_wait = 240  # 4 минуты
    start_time = time.time()
    
    # Правильный эндпоинт в зависимости от типа задачи
    endpoint = f"{MESHY_API_URL}/text-to-3d/{task_id}" if task_type == "text" else f"{MESHY_API_URL}/image-to-3d/{task_id}"
    ...
    while time.time() - start_time < max_wait:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"Статус {task_id} ({task_type}): {status} | {progress}%")
                
                if status == "SUCCEEDED":
                    model_urls = status_data.get("model_urls", {}) or status_data.get("output", {})
                    ...
                    if model_url:
                        ...
                        if model_response.status_code == 200:
                            ...
                            send_document(chat_id, model_response.content, "3d_model.glb",
                                          f"✅ Готово! Осталось сегодня: {remaining}/3")
                            return True
                        else:
                    ...
                    send_message(chat_id, "❌ Генерация не удалась. Попробуй другое фото или промт.")
                    return False
        except Exception as e:
...
def generate_3d_model_text(chat_id, prompt, user_id):
    ...
    if len(prompt.strip()) < 5:
        send_message(chat_id, "📝 Промт слишком короткий. Опиши подробнее.")
        return
    
    send_message(chat_id, f"🎲 Генерация по тексту...\n⏳ 1–4 минуты\nПромт: {prompt[:120]}")
    ...
    wait_for_generation(chat_id, task["task_id"], user_id, task_type="text")
...
def generate_3d_model_image(chat_id, image_url, user_id):
    ...
    send_message(chat_id, "🎨 Генерация по фото...\n⏳ 1–4 минуты")
    ...
    wait_for_generation(chat_id, task["task_id"], user_id, task_type="image")
                ...
                for update in updates:
                    ...
                    if "callback_query" in update:
                        ...
                        if data == "generate":
                            ...
                            send_message(chat_id, "🎲 Введи описание модели:")
                        elif data == "generate_photo":
                            user_data[chat_id] = {"awaiting_photo": True}
                            send_message(chat_id, "📸 Отправь фото объекта.\nПодойдут: игрушки, фигурки, животные, предметы.")
                        elif data == "my_models":
                            send_message(chat_id, "📦 История моделей появится позже.")
                        elif data == "subscription":
                            send_message(chat_id, "💎 Премиум — 299₽/мес (безлимит)\nСкоро будет оплата через Telegram Stars!")
                        ...
                        if "text" in msg and msg["text"] == "/help":
                            send_message(chat_id, "📖 Помощь:\nНажми кнопку и отправь текст или фото.")
                            continue
                        ...
                        # Фото
                            ...
                            if "text" in msg:
                                send_message(chat_id, "Нажми кнопку в меню, чтобы начать генерацию!")
