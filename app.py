...
main_keyboard = {
    "inline_keyboard": [
        ...
        [{"text": "🎟 Разовая генерация (100₽)", "callback_data": "buy_one"}],
        [{"text": "💎 Подписка (299₽/мес)", "callback_data": "subscription"}],
...
def meshy_generate_from_photo(image_base64):
    if not MESHY_API_KEY:
        raise Exception("Meshy API key not configured")
    headers = {"Authorization": f"Bearer {MESHY_API_KEY}", "Content-Type": "application/json"}
    payload = {"image_base64": image_base64, "should_remesh": True}
    resp = requests.post(f"{MESHY_API_URL}/image-to-3d", headers=headers, json=payload, timeout=30)
    if resp.status_code not in (200, 202):
        raise Exception(f"Meshy error: {resp.status_code}")
    task_id = resp.json().get("result")
    if not task_id:
    ...
    while True:
        time.sleep(5)
        r = requests.get(f"{MESHY_API_URL}/image-to-3d/{task_id}", headers=headers)
        if r.status_code == 200:
            data = r.json()
            if data.get("status") == "SUCCEEDED":
                model_url = data.get("model_urls", {}).get("glb")
...
def handle_help_info(chat_id):
    text = (
        ...
        "🔧 *Hunyuan (Точная)* — генерирует инженерные детали по тексту или фото. Высокая точность, подходит для печати функциональных узлов.\n\n"
        "🎟 *Разовая генерация* — 100₽ за модель.\n\n"
        "💎 *Подписка* — 299₽/мес, безлимит.\n\n"
        "📦 *История* — скоро.\n\n"
        "Бесплатно: *3 модели в день*. Остаток сбрасывается каждые 24 часа.\n\n"
        "Просто выбери движок и отправь текст или фото."
    )
    send_message(chat_id, text, keyboard=back_keyboard)


def handle_buy_one(chat_id):
    send_message(chat_id, "🎟 Разовая генерация стоит 100₽.\n\nОплата будет доступна позже. Пока используй бесплатные генерации.", keyboard=back_keyboard)


def handle_subscription(chat_id):
    send_message(chat_id, "💎 Подписка на месяц — 299₽ (безлимит).\n\nОплата появится в ближайшее время.", keyboard=back_keyboard)


def handle_my_models(chat_id):
    send_message(chat_id, "📦 История моделей появится в следующей версии бота.", keyboard=back_keyboard)
                ...
                for update in resp.json().get("result", []):
                    ...
                    if "callback_query" in update:
                        ...
                        if data == "menu":
                            ...
                            handle_buy_one(chat_id)
                        elif data == "subscription":
                            handle_subscription(chat_id)
                        elif data == "my_models":
                            handle_my_models(chat_id)
                        elif data == "gen_meshy":
                        ...
                        if chat_id in user_states:
                            ...
                            if not can_generate(user_id):
                                send_message(chat_id, "❌ Бесплатный лимит на сегодня закончился. Купи разовую генерацию или подписку.", keyboard=main_keyboard)
                                continue
                            
                            if "text" in msg:
                                prompt = msg["text"]
                                send_message(chat_id, f"⏳ Генерирую через {engine.upper()} по тексту... (1-3 мин)")
                                try:
                                    if engine == "meshy":
                                    ...
                                    if model:
                                        ...
                                        send_document(chat_id, model, caption=f"{engine.upper()} (текст): {prompt[:100]}")
                                        send_message(chat_id, "✅ Готово!")
                                    else:
                                        send_message(chat_id, "❌ Ошибка: модель не получена, попробуй ещё раз.")
                                except Exception as e:
                                    send_message(chat_id, f"❌ Ошибка: {str(e)}")
                            
                            elif "photo" in msg:
                                send_message(chat_id, f"⏳ Генерирую через {engine.upper()} по фото... (1-3 мин)")
                                try:
                                ...
                                send_message(chat_id, "Пожалуйста, отправь текст или фото.", keyboard=back_keyboard)
                        else:
...
@flask_app.route('/')
def home():
    return "Bot running (Meshy + Hunyuan + photo)"
