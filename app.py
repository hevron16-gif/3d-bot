...
TEXTS = {
    ...
    "en": {
        ...
        "blueprint_start": "📐 <b>Blueprint Generation Mode</b>\n\nSend <b>THREE</b> photos in order:\n1️⃣ Front view\n2️⃣ Left side view\n3️⃣ Top view\n\n⚠️ Send photos one at a time.",
        "blueprint_1": "✅ Front view received.\n📸 Send <b>left side view</b>.",
        "blueprint_2": "✅ Left view received.\n📸 Send <b>top view</b>.",
    }
}
...
def hunyuan_generate_from_drawings(prompt, front_b64, left_b64, top_b64, output_format='stl', max_retries=3):
    """Генерация по трём чертежам (Multi-View) через ОСНОВНОЙ хост."""
    ...
    for attempt in range(max_retries):
        try:
            ...
            if "Error" in data["Response"]:
                ...
                raise Exception(f"[MultiView] Ошибка API [{err.get('Code')}]: {err.get('Message')}")
                ...
                if "Response" in data:
                    ...
                    if status == "DONE":
                        ...
                        if model_resp.status_code != 200:
                            raise Exception(f"[MultiView] Не удалось скачать модель: HTTP {model_resp.status_code}")


                        model_data = model_resp.content
                        print(f"[MultiView] ✅ Модель готова: {len(model_data)} байт")
...
def process_blueprint_generation(chat_id, user_id, front_b64, left_b64, top_b64, gen_type):
    """Фоновая генерация по трём чертежам."""
    output_format = user_format_choice.get(user_id, "stl")
    prompt = "A 3d model based on three views: front, left side, top"
    try:
        model, fmt = hunyuan_generate_from_drawings(prompt, front_b64, left_b64, top_b64, output_format)
        if model:
            use_generation(user_id, gen_type)
            update_stats(user_id)
            caption = t(user_id, "model_ready", service="HUNYUAN (Multi-View)", fmt=fmt.upper(), prompt="Чертёж")
            if gen_type == "free" and user_id != ADMIN_CHAT_ID:
                remaining = max(0, FREE_LIMIT - user_free_used.get(user_id, 0))
                caption += t(user_id, "free_remaining", remaining=remaining, limit=FREE_LIMIT)
            filename = f"model.{fmt}"
                ...
                for update in updates:
                    ...
                    if "callback_query" in update:
                        ...
                        if data == "menu":
                            ...
                            send_message(chat_id, t(user_id, "blueprint_start"), keyboard=cancel_keyboard)
                        elif data == "cancel":
                    ...
                    if "message" in update:
                        ...
                        if chat_id in user_states:
                            ...
                            if state == "awaiting_text" and "text" in msg:
                                ...
                                count = len(bp_images)
                                if count == 1:
                                    send_message(chat_id, t(user_id, "blueprint_1"))
                                elif count == 2:
                                    send_message(chat_id, t(user_id, "blueprint_2"))
                                elif count >= 3:
                                    del user_states[chat_id]
                                    rate_ok, rate_msg = check_rate_limit(user_id)
                                    if not rate_ok:
