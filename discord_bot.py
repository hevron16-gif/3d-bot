...
print(f"SECRET_ID length: {len(secret_id)}")
print(f"SECRET_ID first 10 chars: {secret_id[:10] if secret_id else 'EMPTY'}")
print(f"SECRET_KEY length: {len(secret_key)}")
print(f"SECRET_KEY exists: {'YES' if secret_key else 'NO'}")


if secret_id:
    print("First 20 chars analysis:")
    for i, char in enumerate(secret_id[:20]):
        print(f"  [{i}]: {repr(char)}")
    if '\x00' in secret_id:
        print("⚠️ NULL BYTE FOUND in SECRET_ID!")
    if '\n' in secret_id:
        print("⚠️ NEWLINE FOUND in SECRET_ID!")
    if ' ' in secret_id:
        print("⚠️ SPACE FOUND in SECRET_ID!")
