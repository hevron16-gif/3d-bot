...
def hunyuan_generate(prompt, max_retries=3):
    """Генерация 3D-модели по тексту с автоматическими повторами"""
    for attempt in range(max_retries):
        try:
            print(f"🟢 [HUNYUAN] Попытка {attempt+1} для промта: {prompt[:50]}...")
                ...
                if "Response" in data:
                    ...
                    if status == "DONE":
                        ...
                        if result_files and "Url" in result_files[0]:
                            ...
                            print(f"🟢 [HUNYUAN] GLB URL: {glb_url}")


                            # Конвертируем GLB → STL
            ...
            if attempt < max_retries - 1:
                ...
                time.sleep(5)
                continue
            raise Exception("Timeout waiting for model")
...
async def generate(interaction: discord.Interaction, prompt: str):
    ...
    if not can_generate(user_id):
        await interaction.response.send_message(
            "❌ Daily free limit (5 models) reached. Support the project via Telegram: @Kostya_3d_bot",
            ephemeral=True
        )
        return
    ...
    await interaction.followup.send(f"🔄 Generating 3D model from prompt:\n`{prompt}`\nPlease wait 1-3 minutes...")
