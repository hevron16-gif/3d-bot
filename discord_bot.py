...
# Включаем ВСЕ намерения
intents = discord.Intents.default()
intents.message_content = True


# Инициализация с поддержкой слэш-команд
                ...
                if "Response" in data:
                    ...
                    if status == "DONE":
                        ...
                        if result_files and "Url" in result_files[0]:
                            ...
                            print("Downloading GLB directly...")
                            ...
                            if model_resp.status_code == 200:
                                ...
                                print(f"Download failed: {model_resp.status_code}")
...
@bot.tree.command(name="generate", description="Generate 3D model from text prompt")
async def generate(interaction: discord.Interaction, prompt: str):
    print(f"!!! COMMAND RECEIVED from {interaction.user.id}: {prompt[:50]}...")
    ...
    try:
        model_data = await hunyuan_generate(prompt)
        use_generation(user_id)
        await interaction.followup.send(file=discord.File(model_data, filename="model.glb"))
...
async def on_interaction(interaction):
    print(f"Interaction received: {interaction.type} from {interaction.user.name}")
