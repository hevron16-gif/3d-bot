...
print(f"SECRET_ID length: {len(TENCENT_SECRET_ID or '')}")
print(f"SECRET_ID first 10 chars: {(TENCENT_SECRET_ID or '')[:10]}")
print(f"SECRET_KEY exists: {'YES' if TENCENT_SECRET_KEY else 'NO'}")
    ...
    for attempt in range(3):
        try:
            ...
            if "Response" not in data or "JobId" not in data["Response"]:
                error = data.get("Response", {}).get("Error", {}).get("Message", "Unknown")
                raise Exception(f"Hunyuan submit error: {error}")
