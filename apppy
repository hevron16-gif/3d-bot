                ...
                if "Response" in data:
                    ...
                    if status == "DONE":
                        ...
                        if result_files and "Url" in result_files[0]:
                            ...
                            # Конвертация в STL через тот же HUNYUAN_HOST
                ...
                if "Response" in data:
                    ...
                    if status == "DONE":
                        ...
                        if result_files and "Url" in result_files[0]:
                            ...
                            # Конвертация в STL через AI3D_HOST (файл на AI3D)
                            conv_action = "Convert3DFormat"
                            conv_payload = json.dumps({"File3D": glb_url, "Format": "STL"})
...
def submit_part_job(file_url, max_retries=2):
    for attempt in range(max_retries):
        try:
...
def process_part_job(chat_id, user_id, file_url, gen_type):
    try:
        ...
        job_id = submit_part_job(file_url)
