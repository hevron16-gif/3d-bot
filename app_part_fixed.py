...
def process_part_generation(chat_id, user_id, glb_url, gen_type, job_id=None):
    """Фоновая разбивка: конвертирует GLB в FBX, затем разбивает на части.

    Сначала пытается перезапросить job_id для получения свежего URL.
    Если Convert3DFormat падает — пробует альтернативные подходы.
    """
    try:
        ...
        # --- Попытка 1: Прямая конвертация через Convert3DFormat ---
        for attempt in range(3):
            try:
                print(f"[Part] Convert3DFormat GLB→FBX, попытка {attempt+1}/3")
                conv_action = "Convert3DFormat"
                ...
                if "Response" in conv_data:
                    ...
                    if "Error" in resp:
                        ...
                        # Если ошибка «url expired» — пробуем ещё раз (может CDN прогрелся)
                    ...
                    if "ResultFile3D" in resp:
                        ...
                        last_error = f"ResultFile3D отсутствует в ответе. Ключи: {list(resp.keys())}"
                        print(f"[Part] {last_error}")
                else:
                    last_error = f"Response отсутствует в ответе. Ключи: {list(conv_data.keys())}"
                    print(f"[Part] {last_error}")

                # ★ Проверка: может быть ответ — это Job (асинхронная операция) ★
                if "Response" in conv_data and "JobId" in conv_data["Response"]:
                    conv_job_id = conv_data["Response"]["JobId"]
                    print(f"[Part] Convert3DFormat вернул JobId={conv_job_id}, опрашиваю...")
                    conv_query_action = "QueryHunyuanTo3DJob"  # или аналогичный
                    conv_query_payload = json.dumps({"JobId": conv_job_id})
                    for _ in range(12):  # 12 × 5 = 60 сек
                        time.sleep(5)
                        conv_query_headers = get_tencent_headers(conv_query_action, conv_query_payload)
                        conv_query_resp = safe_request("post", f"https://{HUNYUAN_HOST}",
                                                       headers=conv_query_headers, data=conv_query_payload, timeout=90)
                        conv_query_data = conv_query_resp.json()
                        print(f"[Part] QueryConvert: {json.dumps(conv_query_data, ensure_ascii=False)[:300]}")
                        if "Response" in conv_query_data:
                            qr = conv_query_data["Response"]
                            if qr.get("Status") == "DONE":
                                if "ResultFile3D" in qr:
                                    fbx_url = qr["ResultFile3D"]
                                    break
                            elif qr.get("Status") == "FAIL":
                                last_error = f"Convert async FAILED: {qr.get('ErrorMessage', '?')}"
                                break
                    if fbx_url:
                        break
        ...
        if not fbx_url:
            raise Exception(
                f"Не удалось конвертировать GLB в FBX после 3 попыток.\n"
                f"Причина: {last_error}\n"
                f"GLB URL: {glb_url[:100]}...\n"
                f"Возможно, URL модели истёк. Сгенерируйте модель заново."
            )
