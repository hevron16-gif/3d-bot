...
def hunyuan_generate_from_multiview(image_urls_list, output_format="stl", max_retries=3):
    """Генерация по MultiView (правильный формат: ViewType + ViewImageUrl)"""
    ...
    for i, url in enumerate(image_urls_list):
        ...
        multiview.append({"ViewType": view_type, "ViewImageUrl": url})
    
    for attempt in range(max_retries):
        try:
            print(f"Hunyuan MultiView: попытка {attempt+1}, фото: {len(multiview)}")
            action = "SubmitHunyuanTo3DProJob"
            payload_dict = {
                "Model": "3.1",
