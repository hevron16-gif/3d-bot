...
# Только один эндпоинт — для текста (работает)
...
def create_generation_task_image(image_url):
    ...
    try:
        # Используем другой эндпоинт для фото
        response = requests.post("https://api.meshy.ai/openapi/v2/image-to-3d", headers=headers, json=payload, timeout=30)
...
# ========== ОЖИДАНИЕ ГЕНЕРАЦИИ (УНИВЕРСАЛЬНАЯ) ==========
def wait_for_generation(chat_id, task_id, user_id, task_type="text"):
    max_wait = 180
    start_time = time.time()
    
    # Выбираем эндпоинт в зависимости от типа задачи
    if task_type == "text":
        endpoint = f"https://api.meshy.ai/openapi/v2/text-to-3d/{task_id}"
    else:
        endpoint = f"https://api.meshy.ai/openapi/v2/image-to-3d/{task_id}"
...
def generate_3d_model_text(chat_id, prompt, user_id):
    ...
    if len(prompt.strip()) < 5:
        send_message(chat_id, "📝 Промт слишком короткий. Опиши подробнее (минимум 5 символов).")
        return
                ...
                for update in updates:
                    ...
                    if "callback_query" in update:
                        ...
                        if data == "generate":
                            ...
                            send_message(chat_id, "📦 История моделей появится позже. Пока сохраняй модели в Telegram.")
                        elif data == "subscription":
                            send_message(chat_id, "💎 Премиум подписка — 299₽/мес\n\nДаёт безлимитную генерацию.\n\nСкоро появится оплата через Telegram Stars!")
                        ...
                        if "text" in msg and msg["text"] == "/help":
                            send_message(chat_id, "📖 Помощь:\n\n1. Нажми 'Генерация по тексту' и напиши описание\n2. Или нажми 'Генерация по фото' и отправь снимок\n3. Жди 1-3 минуты\n4. Скачай модель\n\nБесплатно: 3 модели в день")
                            continue
